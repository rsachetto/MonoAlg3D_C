...
// Model id:
// 1 = ToRORd_fkatp, 2 = ToRORd_dynCl, 3 = ToRORd_Land
struct extra_data_for_torord_general * set_common_torord_general (struct config *config, uint32_t num_cells, uint32_t model_id) {
    struct extra_data_for_torord_general *extra_data = MALLOC_ONE_TYPE(struct extra_data_for_torord_general);

    configure_torord_extra_data_with_user_input(extra_data, config, model_id);
    configure_torord_extra_data_initial_conditions(extra_data, model_id);

    // Allocate the other arrays
    switch (model_id) {
        // ToRORd_fkatp
        case 1: extra_data->transmurality = MALLOC_ARRAY_OF_TYPE(real, num_cells);
                extra_data->sf_IKs = NULL;
                extra_data->basetoapex = NULL;
                break;
        // ToRORd_dynCl
        case 2: extra_data->transmurality = MALLOC_ARRAY_OF_TYPE(real, num_cells);
                extra_data->sf_IKs = NULL;
                extra_data->basetoapex = NULL;
                break;
        // ToRORd_Land
        case 3: extra_data->transmurality = MALLOC_ARRAY_OF_TYPE(real, num_cells);
                extra_data->sf_IKs = MALLOC_ARRAY_OF_TYPE(real, num_cells);
                extra_data->basetoapex = MALLOC_ARRAY_OF_TYPE(real, num_cells);
                break;
        default:
                fprintf(stderr,"[-] ERROR! Invalid value for 'model_id'!\n");
                exit(EXIT_FAILURE);
    }
}
...
void configure_torord_extra_data_with_user_input (struct extra_data_for_torord_general *extra_data, struct config *config, uint32_t model_id) {
    // Get all the current modifiers from the config file
    real INa_Multiplier = 1.0;    
    GET_PARAMETER_NUMERIC_VALUE_OR_USE_DEFAULT(real, INa_Multiplier, config, "INa_Multiplier");
    real INaL_Multiplier = 1.0;    
    GET_PARAMETER_NUMERIC_VALUE_OR_USE_DEFAULT(real, INaL_Multiplier, config, "INaL_Multiplier");
    ...
    // Get the number of equations based on the 'model_id'
    uint32_t neq;
    switch (model_id) {
        // ToRORd_fkatp
        case 1: neq = 43;
...
void configure_torord_extra_data_initial_conditions (struct extra_data_for_torord_general *extra_data, uint32_t model_id) {
    switch (model_id) {
        // ToRORd_fkatp
        case 1: printf("[extra_data] Setting 'ToRORd_fkatp' initial conditions ...\n");
                configure_torord_fkatp_initial_conditions(extra_data);
                break;
        // ToRORd_dynCl
        case 2: printf("[extra_data] Setting 'ToRORd_dynCl' initial conditions ...\n");
                configure_torord_dynCl_initial_conditions(extra_data);
                break;
        // ToRORd_Land
        case 3: printf("[extra_data] Setting 'ToRORd_Land' initial conditions ...\n");
                configure_torord_Land_initial_conditions(extra_data);
                break;
        default:
                fprintf(stderr,"[-] ERROR! Invalid value for 'model_id'!\n");
                exit(EXIT_FAILURE);
