...
#define EUCLIDIAN_DISTANCE(p, q)                                               \
  sqrt(pow(p.x - q.x, 2.0) + pow(p.y - q.y, 2.0) + pow(p.z - q.z, 2.0))

void fill_element(uint32_t position, real_cpu dx, real_cpu dy, real_cpu dz,
...
static void
fill_discretization_matrix_elements(struct cell_node *grid_cell,
  ...
  uint16_t neighbour_grid_cell_level =
      ((struct basic_cell_data *)(neighbour_grid_cell))->level;
  enum cell_type neighbour_grid_cell_type =
      ((struct basic_cell_data *)(neighbour_grid_cell))->type;
...
static void get_leads(struct config *config) {
  ...
  while (true) {
    ...
    struct point_3d coord =
        POINT3D(leads_coords[0], leads_coords[1], leads_coords[2]);
...
INIT_CALC_ECG(init_pseudo_bidomain_cpu) {
  config->persistent_data =
      CALLOC_ONE_TYPE(struct pseudo_bidomain_persistent_data);
  ...
  GET_PARAMETER_NUMERIC_VALUE_OR_REPORT_ERROR(real_cpu, sigma_b, config,
                                              "sigma_b");
  ...
  PSEUDO_BIDOMAIN_DATA->distances =
      MALLOC_ARRAY_OF_TYPE(real, PSEUDO_BIDOMAIN_DATA->n_leads * n_active);
  ...
  for (uint32_t i = 0; i < PSEUDO_BIDOMAIN_DATA->n_leads; i++) {
    ...
    struct point_3d lead = PSEUDO_BIDOMAIN_DATA->leads[i];
        ...
        for (int j = 0; j < n_active; j++) {
          ...
          PSEUDO_BIDOMAIN_DATA->distances[index] =
              EUCLIDIAN_DISTANCE(lead, center);
        }
  }
...
CALC_ECG(pseudo_bidomain_cpu) {
  // use the equation described in
  // https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3378475/#FD7
  uint32_t n_active = the_grid->num_active_cells;
  struct cell_node **ac = the_grid->active_cells;
    ...
    for (uint32_t i = 0; i < n_active; i++) {
      ...
      for (size_t el = 0; el < max_el; el++) {
        PSEUDO_BIDOMAIN_DATA->beta_im[i] +=
            cell_elements[el].value_ecg * cell_elements[el].cell->v / volume;
      }
    }
    ...
    for (uint32_t i = 0; i < PSEUDO_BIDOMAIN_DATA->n_leads; i++) {
      real_cpu local_sum = 0.0;
        ...
        for (uint32_t j = 0; j < n_active; j++) {
          ...
          uint32_t index = i * n_active + j;
          local_sum += ((PSEUDO_BIDOMAIN_DATA->beta_im[j] /
                         PSEUDO_BIDOMAIN_DATA->distances[index])) *
                       volume;
        }

        fprintf(PSEUDO_BIDOMAIN_DATA->output_file, "%lf ",
                -PSEUDO_BIDOMAIN_DATA->scale_factor * local_sum);
    }
...
INIT_CALC_ECG(init_pseudo_bidomain_gpu) {
  ...
  if (!the_ode_solver->gpu) {
    log_warn("The current implementation of pseudo_bidomain_gpu only works "
             "when the odes are also being solved using the GPU! Falling back "
             "to CPU version\n");
    shput(config->config_data, "use_gpu", "false");
    return;
  }

  // This is allocated when using the CPU code, but we do not need it in the gpu
  // version
  free(PSEUDO_BIDOMAIN_DATA->beta_im);
  ...
  // We need to do that because val is originally a float value. Maybe we should
  // make it real.
  real *new_val = NULL;
  ...
  check_cuda_error(
      cudaMalloc((void **)&(PSEUDO_BIDOMAIN_DATA->d_col), nz * sizeof(int)));
  check_cuda_error(cudaMalloc((void **)&(PSEUDO_BIDOMAIN_DATA->d_row),
                              (N + 1) * sizeof(int)));
  check_cuda_error(
      cudaMalloc((void **)&(PSEUDO_BIDOMAIN_DATA->d_val), nz * sizeof(real)));
  check_cuda_error(
      cudaMalloc((void **)&(PSEUDO_BIDOMAIN_DATA->beta_im), N * sizeof(real)));
  check_cuda_error(
      cudaMalloc((void **)&(PSEUDO_BIDOMAIN_DATA->d_distances),
                 PSEUDO_BIDOMAIN_DATA->n_leads * N * sizeof(real)));
  check_cuda_error(cudaMalloc((void **)&(PSEUDO_BIDOMAIN_DATA->d_volumes),
                              N * sizeof(real)));
  check_cuda_error(
      cudaMalloc((void **)&(PSEUDO_BIDOMAIN_DATA->tmp_data), N * sizeof(real)));

#if CUBLAS_VER_MAJOR >= 11
  check_cuda_error(cusparseCreateCsr(
      &(PSEUDO_BIDOMAIN_DATA->matA), N, N, nz, PSEUDO_BIDOMAIN_DATA->d_row,
      PSEUDO_BIDOMAIN_DATA->d_col, PSEUDO_BIDOMAIN_DATA->d_val,
      CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO,
      CUBLAS_SIZE));
  check_cuda_error(cusparseCreateDnVec(&(PSEUDO_BIDOMAIN_DATA->vec_beta_im), N,
                                       PSEUDO_BIDOMAIN_DATA->beta_im,
                                       CUBLAS_SIZE));
  check_cuda_error(cusparseCreateDnVec(&(PSEUDO_BIDOMAIN_DATA->vec_vm), N,
                                       the_ode_solver->sv, CUBLAS_SIZE));
#else
  check_cuda_error(
      (cudaError_t)cusparseCreateMatDescr(&(PSEUDO_BIDOMAIN_DATA->descr)));
  cusparseSetMatType(PSEUDO_BIDOMAIN_DATA->descr, CUSPARSE_MATRIX_TYPE_GENERAL);
  cusparseSetMatIndexBase(PSEUDO_BIDOMAIN_DATA->descr,
                          CUSPARSE_INDEX_BASE_ZERO);
  PSEUDO_BIDOMAIN_DATA->local_sv = the_ode_solver->sv;
#endif

  check_cuda_error(cudaMemcpy(PSEUDO_BIDOMAIN_DATA->d_col, J, nz * sizeof(int),
                              cudaMemcpyHostToDevice)); // JA
  check_cuda_error(cudaMemcpy(PSEUDO_BIDOMAIN_DATA->d_row, I,
                              (N + 1) * sizeof(int),
                              cudaMemcpyHostToDevice)); // IA
  check_cuda_error(cudaMemcpy(PSEUDO_BIDOMAIN_DATA->d_val, new_val,
                              nz * sizeof(real), cudaMemcpyHostToDevice)); // A
  check_cuda_error(cudaMemcpy(PSEUDO_BIDOMAIN_DATA->d_distances,
                              PSEUDO_BIDOMAIN_DATA->distances,
                              PSEUDO_BIDOMAIN_DATA->n_leads * N * sizeof(real),
  ...
  struct cell_node **ac = the_grid->active_cells;
    ...
    check_cuda_error(cudaMemcpy(PSEUDO_BIDOMAIN_DATA->d_volumes,
                                PSEUDO_BIDOMAIN_DATA->volumes, N * sizeof(real),
                                cudaMemcpyHostToDevice));
    ...
    check_cuda_error(cusparseSpMV_bufferSize(
        PSEUDO_BIDOMAIN_DATA->cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
        &alpha, PSEUDO_BIDOMAIN_DATA->matA, PSEUDO_BIDOMAIN_DATA->vec_vm, &beta,
        PSEUDO_BIDOMAIN_DATA->vec_beta_im, CUBLAS_SIZE, CUSPARSE_ALG,
        &(PSEUDO_BIDOMAIN_DATA->bufferSize)));

    check_cuda_error(cudaMalloc(&(PSEUDO_BIDOMAIN_DATA->buffer),
                                PSEUDO_BIDOMAIN_DATA->bufferSize));
#endif
...
CALC_ECG(pseudo_bidomain_gpu) {
  ...
  real beta = 0.0;
#if CUSPARSE_VER_MAJOR >= 11
  check_cublas_error(cusparseSpMV(
      PSEUDO_BIDOMAIN_DATA->cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
      &alpha, PSEUDO_BIDOMAIN_DATA->matA, PSEUDO_BIDOMAIN_DATA->vec_vm, &beta,
      PSEUDO_BIDOMAIN_DATA->vec_beta_im, CUBLAS_SIZE, CUSPARSE_ALG,
...
#ifdef CELL_MODEL_REAL_DOUBLE
  cusparseDcsrmv(PSEUDO_BIDOMAIN_DATA->cusparseHandle,
                 CUSPARSE_OPERATION_NON_TRANSPOSE, n_active, n_active,
                 PSEUDO_BIDOMAIN_DATA->nz, &alpha, PSEUDO_BIDOMAIN_DATA->descr,
                 PSEUDO_BIDOMAIN_DATA->d_val, PSEUDO_BIDOMAIN_DATA->d_row,
                 PSEUDO_BIDOMAIN_DATA->d_col, PSEUDO_BIDOMAIN_DATA->local_sv,
                 &beta, PSEUDO_BIDOMAIN_DATA->beta_im);
#else
  cusparseScsrmv(PSEUDO_BIDOMAIN_DATA->cusparseHandle,
                 CUSPARSE_OPERATION_NON_TRANSPOSE, n_active, n_active,
                 PSEUDO_BIDOMAIN_DATA->nz, &alpha, PSEUDO_BIDOMAIN_DATA->descr,
                 PSEUDO_BIDOMAIN_DATA->d_val, PSEUDO_BIDOMAIN_DATA->d_row,
                 PSEUDO_BIDOMAIN_DATA->d_col, PSEUDO_BIDOMAIN_DATA->local_sv,
  ...
  gpu_vec_div_vec(PSEUDO_BIDOMAIN_DATA->beta_im,
                  PSEUDO_BIDOMAIN_DATA->d_volumes,
                  PSEUDO_BIDOMAIN_DATA->beta_im, n_active);

  for (int i = 0; i < PSEUDO_BIDOMAIN_DATA->n_leads; i++) {

    // beta_im / distance
    gpu_vec_div_vec(PSEUDO_BIDOMAIN_DATA->beta_im,
                    PSEUDO_BIDOMAIN_DATA->d_distances + n_active * i,
                    PSEUDO_BIDOMAIN_DATA->tmp_data, n_active);
    real local_sum;

#ifdef CELL_MODEL_REAL_DOUBLE
    check_cublas_error(cublasDdot(PSEUDO_BIDOMAIN_DATA->cublasHandle, n_active,
                                  PSEUDO_BIDOMAIN_DATA->tmp_data, 1,
                                  PSEUDO_BIDOMAIN_DATA->d_volumes, 1,
                                  &local_sum));
#else
    check_cublas_error(cublasSdot(PSEUDO_BIDOMAIN_DATA->cublasHandle, n_active,
                                  PSEUDO_BIDOMAIN_DATA->tmp_data, 1,
                                  PSEUDO_BIDOMAIN_DATA->d_volumes, 1,
                                  &local_sum));
#endif
...
END_CALC_ECG(end_pseudo_bidomain_gpu) {
  ...
  struct pseudo_bidomain_persistent_data *persistent_data =
      (struct pseudo_bidomain_persistent_data *)config->persistent_data;

  if (!persistent_data)
    return;

  check_cuda_error(
      (cudaError_t)cusparseDestroy(persistent_data->cusparseHandle));
...
#else
  check_cuda_error(
      (cudaError_t)cusparseDestroyMatDescr(persistent_data->descr));
#endif
...
#else
    log_warn("Cuda runtime not found in this system. Falling back to CPU "
             "version!!\n");
    init_pseudo_bidomain_cpu(config, NULL, the_grid, output_dir);
#endif
  } else {
...
INIT_CALC_ECG(init_pseudo_bidomain_with_diffusive_current_cpu) {
  ...
  GET_PARAMETER_NUMERIC_VALUE_OR_REPORT_ERROR(uint32_t, diff_curr_rate, config,
                                              "diff_curr_rate");
  PSEUDO_BIDOMAIN_DATA->diff_curr_rate = diff_curr_rate;
  real_cpu diff_curr_max_time = 100.0;
  GET_PARAMETER_NUMERIC_VALUE_OR_REPORT_ERROR(real_cpu, diff_curr_max_time,
                                              config, "diff_curr_max_time");
...
CALC_ECG(pseudo_bidomain_with_diffusive_current_cpu) {
  ...
  struct cell_node **ac = the_grid->active_cells;
    ...
    if (time_info->iteration % PSEUDO_BIDOMAIN_DATA->diff_curr_rate == 0 &&
        time_info->current_t <= PSEUDO_BIDOMAIN_DATA->diff_curr_max_time) {
      char filename_diffusive_current[200];
      sprintf(filename_diffusive_current, "%s/diffusive_current_t_%g_ms.txt",
              output_dir, time_info->current_t);
...
INIT_CALC_ECG(init_pseudo_bidomain_with_diffusive_current_gpu) {
  ...
  init_pseudo_bidomain_with_diffusive_current_cpu(config, NULL, the_grid,
                                                  output_dir);
...
CALC_ECG(pseudo_bidomain_with_diffusive_current_gpu) {
  ...
  if (time_info->iteration % PSEUDO_BIDOMAIN_DATA->diff_curr_rate == 0 &&
    ...
    check_cuda_error(cudaMemcpy(
        PSEUDO_BIDOMAIN_DATA->beta_im_cpu, PSEUDO_BIDOMAIN_DATA->beta_im,
        n_active * sizeof(real), cudaMemcpyDeviceToHost));
    for (uint32_t i = 0; i < n_active; i++) {
      fprintf(output_diffusive_current, "%g\n",
