...
void print_linear_system_solver_config_values(struct config* s) {
    ...
    print_to_stdout_and_file("Linear system solver main function: %s\n", s->main_function_name);

    if(s->init_function_name)
        print_to_stdout_and_file("Linear system solver init function: %s\n", s->init_function_name);

    if(s->end_function_name)
        print_to_stdout_and_file("Liner system solver end function: %s\n", s->end_function_name);
