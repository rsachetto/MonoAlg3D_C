...
void print_update_monodomain_config_values(struct config* s) {
    ...
    print_to_stdout_and_file("Update Monodomain main function: %s\n", s->main_function_name);

    if(s->init_function_name)
        print_to_stdout_and_file("Update Monodomain init function: %s\n", s->init_function_name);

    if(s->end_function_name)
        print_to_stdout_and_file("Update Monodomain end function: %s\n", s->end_function_name);
