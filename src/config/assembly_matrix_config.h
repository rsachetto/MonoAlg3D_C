...
#define ASSEMBLY_MATRIX(name) EXPORT_FN void name(struct config *config, struct monodomain_solver *the_solver, struct grid *the_grid)
typedef ASSEMBLY_MATRIX(assembly_matrix_fn);

#define INIT_ASSEMBLY_MATRIX(name) EXPORT_FN void name(struct config *config, struct monodomain_solver *the_solver, struct grid *the_grid, real_cpu initial_v, real_cpu purkinje_initial_v)
