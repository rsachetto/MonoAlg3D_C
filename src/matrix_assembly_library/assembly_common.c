...
// IMPORTANT USAGE:
//   - If CELL_BZ_FACTOR(c) is left as 1.0, this file is algebraically identical
//     to the original code.
//   - For the BZ run, cell->sigma should contain the unscaled/base tensor sigma0.
//     The BZ attenuation is applied here only through F_face.
//   - Do NOT use this together with sigma already multiplied by F, otherwise the
//     BZ effect is applied twice.
//
#ifndef CELL_BZ_FACTOR
#define CELL_BZ_FACTOR(c) (1.0)
// #define CELL_BZ_FACTOR(c) (HU_WEIGHT_BORDER_ZONE(c))
#endif
...
// -----------------------------------------------------------------------------
// Border-zone scalar face multiplier
// -----------------------------------------------------------------------------
// Minimal version: keep the original anisotropic stencil unchanged and multiply
// the entire face tensor row by a scalar face factor F_face = H(Fp, Fn).
//

static inline real_cpu bz_face_multiplier(const struct cell_node *P, const struct cell_node *N) {
    const real_cpu eps = 1e-18;

    const real_cpu Fp = CELL_BZ_FACTOR(P);
    const real_cpu Fn = CELL_BZ_FACTOR(N);

    if(Fp <= eps || Fn <= eps) return 0.0;

    return (2.0 * Fp * Fn) / (Fp + Fn);
}
...
static inline void assemble_x_face_flux(struct cell_node *P, struct cell_node *N, bool is_right_face) {
    ...
    const real_cpu F_face = bz_face_multiplier(P, N);
    const real_cpu s_xx = F_face * harmonic_mean_2_pos(P->sigma.x,  N->sigma.x);
    const real_cpu s_xy = F_face * arithmetic_mean_2(P->sigma.xy, N->sigma.xy);
    const real_cpu s_xz = F_face * arithmetic_mean_2(P->sigma.xz, N->sigma.xz);
...
static inline void assemble_y_face_flux(struct cell_node *P, struct cell_node *N, bool is_top_face) {
    ...
    const real_cpu F_face = bz_face_multiplier(P, N);
    const real_cpu s_yy = F_face * harmonic_mean_2_pos(P->sigma.y,  N->sigma.y);
    const real_cpu s_xy = F_face * arithmetic_mean_2(P->sigma.xy, N->sigma.xy);
    const real_cpu s_yz = F_face * arithmetic_mean_2(P->sigma.yz, N->sigma.yz);
...
static inline void assemble_z_face_flux(struct cell_node *P, struct cell_node *N, bool is_front_face) {
    ...
    const real_cpu F_face = bz_face_multiplier(P, N);
    const real_cpu s_zz = F_face * harmonic_mean_2_pos(P->sigma.z,  N->sigma.z);
    const real_cpu s_xz = F_face * arithmetic_mean_2(P->sigma.xz, N->sigma.xz);
    const real_cpu s_yz = F_face * arithmetic_mean_2(P->sigma.yz, N->sigma.yz);
