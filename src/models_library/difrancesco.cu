...
extern "C" SET_ODE_INITIAL_CONDITIONS_GPU(set_model_initial_conditions_gpu) {
    ...
    print_to_stdout_and_file("Using DiFrancesco & Noble 1985 GPU model\n");
...
inline __device__ void RHS_gpu(real *sv_, real *rDY_, real stim_current, int threadID_) {
    ...
    CONSTANTS[46] =  3.14159*powf(CONSTANTS[33], 2.00000)*CONSTANTS[34];
    ...
    ALGEBRAIC[34] = ( CONSTANTS[23]*( exp(( CONSTANTS[25]*(CONSTANTS[22] - 2.00000)*STATES[0])/CONSTANTS[45])*powf(STATES[3], CONSTANTS[22])*CONSTANTS[18] -  exp(( (CONSTANTS[25] - 1.00000)*(CONSTANTS[22] - 2.00000)*STATES[0])/CONSTANTS[45])*powf(CONSTANTS[8], CONSTANTS[22])*STATES[6]))/( (1.00000+ CONSTANTS[24]*( STATES[6]*powf(CONSTANTS[8], CONSTANTS[22])+ CONSTANTS[18]*powf(STATES[3], CONSTANTS[22])))*(1.00000+STATES[6]/0.00690000));
    ALGEBRAIC[35] =  CONSTANTS[45]*log((CONSTANTS[8]+ 0.120000*STATES[1])/(STATES[3]+ 0.120000*STATES[2]));
    ALGEBRAIC[36] =  CONSTANTS[26]*powf(STATES[8], 3.00000)*STATES[9]*(STATES[0] - ALGEBRAIC[35]);
    ...
    ALGEBRAIC[44] = ( (( 2.00000*1.00000*CONSTANTS[49]*CONSTANTS[2])/( 1.00000*CONSTANTS[40]))*STATES[14]*powf(STATES[6], CONSTANTS[41]))/(powf(STATES[6], CONSTANTS[41])+powf(CONSTANTS[37], CONSTANTS[41]));
