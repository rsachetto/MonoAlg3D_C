...
__global__ void kernel_set_model_inital_conditions(real *sv, real *initial_conditions, int num_volumes) {
    ...
    if(threadID < num_volumes) {
        ...
        if(initial_conditions == NULL) {
            ...
            *((real *)((char *)sv + pitch * 0) + threadID) = initial_conditions[0];   // V;       millivolt
            *((real *)((char *)sv + pitch * 1) + threadID) = initial_conditions[1];   // Xr1;     dimensionless
            *((real *)((char *)sv + pitch * 2) + threadID) = initial_conditions[2];   // Xr2;     dimensionless
            *((real *)((char *)sv + pitch * 3) + threadID) = initial_conditions[3];   // Xs;      dimensionless
            *((real *)((char *)sv + pitch * 4) + threadID) = initial_conditions[4];   // m;       dimensionless
            *((real *)((char *)sv + pitch * 5) + threadID) = initial_conditions[5];   // h;       dimensionless
            *((real *)((char *)sv + pitch * 6) + threadID) = initial_conditions[6];   // j;       dimensionless
            *((real *)((char *)sv + pitch * 7) + threadID) = initial_conditions[7];   // d;       dimensionless
            *((real *)((char *)sv + pitch * 8) + threadID) = initial_conditions[8];   // f;       dimensionless
            *((real *)((char *)sv + pitch * 9) + threadID) = initial_conditions[9];   // f2;      dimensionless
            *((real *)((char *)sv + pitch * 10) + threadID) = initial_conditions[10]; // fCass;   dimensionless
            *((real *)((char *)sv + pitch * 11) + threadID) = initial_conditions[11]; // s;       dimensionless
            *((real *)((char *)sv + pitch * 12) + threadID) = initial_conditions[12]; // r;       dimensionless
            *((real *)((char *)sv + pitch * 13) + threadID) = initial_conditions[13]; // Ca_i;    millimolar
            *((real *)((char *)sv + pitch * 14) + threadID) = initial_conditions[14]; // Ca_SR;   millimolar
            *((real *)((char *)sv + pitch * 15) + threadID) = initial_conditions[15]; // Ca_ss;   millimolar
            *((real *)((char *)sv + pitch * 16) + threadID) = initial_conditions[16]; // R_prime; dimensionless
            *((real *)((char *)sv + pitch * 17) + threadID) = initial_conditions[17]; // Na_i;    millimolar
            *((real *)((char *)sv + pitch * 18) + threadID) = initial_conditions[18]; // K_i;     millimolar
        }
    }
}
...
inline __device__ void RHS_gpu(real *sv_, real *rDY_, real stim_current, int threadID_, real dt) {
    ...
    const real f = *((real *)((char *)sv_ + pitch * 8) + threadID_);
    ...
    real beta_K1 = ((3.0f * expf(0.0002f * ((V - EK) + 100.0f))) + expf(0.1f * ((V - EK) - 10.0f))) /
                   (1.0f + expf((-0.5f) * (V - EK)));
    ...
    real ICaL = (V < 15.0f - 1.0e-5f || V > 15.0f + 1.0e-5f)
                    ? ((((g_CaL * d * f * f2 * fCass * 4.0f * (V - 15.0f) * powf(F, 2.0f)) / (R * T)) *
                        ((0.25f * Ca_ss * expf((2.0f * (V - 15.0f) * F) / (R * T))) - Cao)) /
                       (expf((2.0f * (V - 15.0f) * F) / (R * T)) - 1.0f))
                    : g_CaL * d * f * f2 * fCass * 2.0f * F * (0.25f * Ca_ss - Cao);
    real IbCa = g_bca * (V - ECa);
    real IpCa = (g_pCa * Ca_i) / (Ca_i + K_pCa);

    real INaK = ((((P_NaK * Ko) / (Ko + K_mk)) * Na_i) / (Na_i + K_mNa)) /
                (1.0f + (0.1245f * expf(((-0.1f) * V * F) / (R * T))) + (0.0353f * expf(((-V) * F) / (R * T))));
    real INa = g_Na * powf(m, 3.0f) * h * j * (V - ENa);
    real IbNa = g_bna * (V - ENa);
    real INaCa = (K_NaCa * ((expf((gamma * V * F) / (R * T)) * powf(Na_i, 3.0f) * Cao) -
                            (expf(((gamma - 1.0f) * V * F) / (R * T)) * powf(Nao, 3) * Ca_i * alpha))) /
                 ((powf(Km_Nai, 3.0f) + powf(Nao, 3.0f)) * (Km_Ca + Cao) *
                  (1.0f + (K_sat * expf(((gamma - 1.0f) * V * F) / (R * T)))));
    ...
    real beta_h = (V < (-40.0f)) ? ((2.7f * expf(0.079f * V)) + (310000.0f * expf(0.3485f * V)))
                                 : (0.77f / (0.13f * (1.0f + expf((V + 10.66f) / (-11.1f)))));
    real tau_h = 1.0f / (alpha_h + beta_h);

    real j_inf = 1.0f / powf(1.0f + expf((V + 71.55f) / 7.43f), 2.0f);
    real alpha_j =
        (V < (-40.0f))
            ? ((((((-25428.0f) * expf(0.2444f * V)) - (6.948e-06f * expf((-0.04391f) * V))) * (V + 37.78f)) / 1.0f) /
               (1.0f + expf(0.311f * (V + 79.23f))))
            : 0.0f;
    real beta_j = (V < (-40.0f)) ? ((0.02424f * expf((-0.01052f) * V)) / (1.0f + expf((-0.1378f) * (V + 40.14f))))
                                 : ((0.6f * expf(0.057f * V)) / (1.0f + expf((-0.1f) * (V + 32.0f))));
    ...
    real tau_f = (1102.5f * expf((-powf(V + 27.0f, 2.0f)) / 225.0f)) + (200.0f / (1.0f + expf((13.0f - V) / 10.0f))) +
                 (180.0f / (1.0f + expf((V + 30.0f) / 10.0f))) + 20.0f;

    real f2_inf = (0.67f / (1.0f + expf((V + 35.0f) / 7.0f))) + 0.33f;
    real tau_f2 = (562.0f * expf((-powf(V + 27.0f, 2.0f)) / 240.0f)) + (31.0f / (1.0f + expf((25.0f - V) / 10.0f))) +
                  (80.0f / (1.0f + expf((V + 30.0f) / 10.0f)));
    ...
    real Ca_i_bufc = 1.0f / (1.0f + ((var_calcium_dynamics__Buf_c * var_calcium_dynamics__K_buf_c) /
                                     powf(Ca_i + var_calcium_dynamics__K_buf_c, 2)));
    const real var_calcium_dynamics__K_buf_sr = 0.3f;
    const real var_calcium_dynamics__Buf_sr = 10.0f;
    real var_calcium_dynamics__Ca_sr_bufsr =
        1.0f / (1.0f + ((var_calcium_dynamics__Buf_sr * var_calcium_dynamics__K_buf_sr) /
                        powf(Ca_SR + var_calcium_dynamics__K_buf_sr, 2)));
    const real var_calcium_dynamics__Buf_ss = 0.4f;
    const real var_calcium_dynamics__K_buf_ss = 0.00025f;
    real Ca_ss_bufss = 1.0f / (1.0f + ((var_calcium_dynamics__Buf_ss * var_calcium_dynamics__K_buf_ss) /
                                       powf(Ca_ss + var_calcium_dynamics__K_buf_ss, 2.0f)));
    ...
    real d_dt_V =
        -(IK1 + Ito + IKr + IKs + ICaL + INaK + INa + IbNa + INaCa + IbCa + IpK + IpCa + var_membrane__i_Stim);

    real d_dt_R_prime = ((-var_calcium_dynamics__k2) * Ca_ss * R_prime) + (var_calcium_dynamics__k4 * (1.0f - R_prime));
    real d_dt_Ca_i =
        Ca_i_bufc *
        (((((var_calcium_dynamics__i_leak - var_calcium_dynamics__i_up) * var_calcium_dynamics__V_sr) /
           var_calcium_dynamics__V_c) +
          var_calcium_dynamics__i_xfer) -
         ((((var_calcium_dynamics__IbCa + var_calcium_dynamics__IpCa) - (2.0f * var_calcium_dynamics__INaCa)) *
           var_calcium_dynamics__Cm) /
          (2.0f * var_calcium_dynamics__V_c * var_calcium_dynamics__F)));
    real d_dt_Ca_SR =
        var_calcium_dynamics__Ca_sr_bufsr * (var_calcium_dynamics__i_up - (Irel + var_calcium_dynamics__i_leak));
    real d_dt_Ca_ss =
        Ca_ss_bufss * (((((-var_calcium_dynamics__ICaL) * var_calcium_dynamics__Cm) /
                         (2.0f * var_calcium_dynamics__V_ss * var_calcium_dynamics__F)) +
                        ((Irel * var_calcium_dynamics__V_sr) / var_calcium_dynamics__V_ss)) -
                       ((var_calcium_dynamics__i_xfer * var_calcium_dynamics__V_c) / var_calcium_dynamics__V_ss));
