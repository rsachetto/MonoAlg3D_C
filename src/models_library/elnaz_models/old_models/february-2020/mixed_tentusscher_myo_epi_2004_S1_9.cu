...
#include "mixed_tentusscher_myo_epi_2004_S1_9.h"
...
__global__ void kernel_set_model_inital_conditions(real *sv, uint32_t *mapping, int num_volumes) 
{
    ...
    if (threadID < num_volumes) 
    {
        ...
        */
            // Elnaz's steady-state initial conditions
            real sv_sst[]={86.5192044363143,0.00130365206656861,0.778502731472955,0.778360844809338,0.000175984539756321,0.484579654828975,0.00295059008531154,0.999998331811119,1.94936501541678e-08,1.90347500274802e-05,0.999774247293631,1.00635124808035,0.999968142119565,6.23120479006420e-05,0.705915347274303,8.69072039411943,140.772003870745};            for (uint32_t i = 0; i < NEQ; i++)
                *((real * )((char *) sv + pitch * i) + threadID) = sv_sst[i];
        }
    }
}
...
//Parameters for IpK;
    ...
    real parameters []={13.4527957311516,0.000441059249410313,0.000162008720309023,0.000621621426165240,0.299853311021211,0.156744006463674,0.193879092011969,3.93737874052701,0.0203695197691590,3.25098207932026,1033.30721827671,0.000514541985880426,0.159908894280276,0.0147964022497568,0.00313060306080565,5.65241254768756e-05};
    GNa=parameters[0];
    GbNa=parameters[1];
    GCaL=parameters[2];
    GbCa=parameters[3];
