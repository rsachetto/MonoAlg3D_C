...
__global__ void kernel_set_model_inital_conditions(real *sv, uint32_t *mapping, int num_volumes) 
{
    ...
    if (threadID < num_volumes) 
    {
        ...
        */
            // Elnaz's steady-state initial conditions
            real sv_sst[]={-86.5934020417845,0.00128316160896992,0.780329095939604,0.780220566229028,0.000174014290922046,0.485351964461050,0.00293503332848286,0.999998357040599,1.92538423669596e-08,1.88473554467734e-05,0.999772914410323,1.00703682498466,0.999994463032827,4.65490502991951e-05,0.633301730023318,9.92651972626448,139.581508364500};            for (uint32_t i = 0; i < NEQ; i++)
                *((real * )((char *) sv + pitch * i) + threadID) = sv_sst[i];
        }
    }
}
