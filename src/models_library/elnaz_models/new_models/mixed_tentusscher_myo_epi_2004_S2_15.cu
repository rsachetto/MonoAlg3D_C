...
__global__ void kernel_set_model_inital_conditions(real *sv, uint32_t *mapping, int num_volumes) 
{
    ...
    if (threadID < num_volumes) 
    {
        ...
        */
            // Elnaz's steady-state initial conditions
            real sv_sst[]={-86.3587896185485,0.00134905676198612,0.774520199073544,0.774374680229778,0.000180109822242613,0.482910475890506,0.00298448938077068,0.999998277316546,2.00218048619546e-08,1.94462488964510e-05,0.999771668739369,1.00653067523994,0.999978756030136,5.82428650958820e-05,0.509754826080024,10.0682799523111,139.518219822205};
for (uint32_t i = 0; i < NEQ; i++)
                *((real * )((char *) sv + pitch * i) + threadID) = sv_sst[i];
        }
    }
