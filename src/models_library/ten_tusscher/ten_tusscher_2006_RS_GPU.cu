...
inline __device__ void RHS_gpu(real *sv_, real *rDY_, real stim_current, int threadID_, real dt) {
    ...
    real beta_K1 = ((3.0f * exp(0.0002f * ((V - EK) + 100.0f))) + exp(0.1f * ((V - EK) - 10.0f))) /
                   (1.0f + exp((-0.5f) * (V - EK)));
    ...
    real ICaL = (V < 15.0f - 1.0e-5f || V > 15.0f + 1.0e-5f)
                    ? ((((g_CaL * d * f * f2 * fCass * 4.0f * (V - 15.0f) * pow(F, 2.0f)) / (R * T)) *
                        ((0.25f * Ca_ss * exp((2.0f * (V - 15.0f) * F) / (R * T))) - Cao)) /
                       (exp((2.0f * (V - 15.0f) * F) / (R * T)) - 1.0f))
    ...
    real INaK = ((((P_NaK * Ko) / (Ko + K_mk)) * Na_i) / (Na_i + K_mNa)) /
                (1.0f + (0.1245f * exp(((-0.1f) * V * F) / (R * T))) + (0.0353f * exp(((-V) * F) / (R * T))));
    real INa = g_Na * pow(m, 3.0f) * h * j * (V - ENa);
    real IbNa = g_bna * (V - ENa);
    real INaCa = (K_NaCa * ((exp((gamma * V * F) / (R * T)) * pow(Na_i, 3.0f) * Cao) -
                            (exp(((gamma - 1.0f) * V * F) / (R * T)) * pow(Nao, 3) * Ca_i * alpha))) /
                 ((pow(Km_Nai, 3.0f) + pow(Nao, 3.0f)) * (Km_Ca + Cao) *
                  (1.0f + (K_sat * exp(((gamma - 1.0f) * V * F) / (R * T)))));
    ...
    real beta_h = (V < (-40.0f)) ? ((2.7f * exp(0.079f * V)) + (310000.0f * exp(0.3485f * V)))
                                 : (0.77f / (0.13f * (1.0f + exp((V + 10.66f) / (-11.1f)))));
    real tau_h = 1.0f / (alpha_h + beta_h);

    real j_inf = 1.0f / pow(1.0f + exp((V + 71.55f) / 7.43f), 2.0f);
    real alpha_j =
        (V < (-40.0f))
            ? ((((((-25428.0f) * exp(0.2444f * V)) - (6.948e-06f * exp((-0.04391f) * V))) * (V + 37.78f)) / 1.0f) /
               (1.0f + exp(0.311f * (V + 79.23f))))
            : 0.0f;
    real beta_j = (V < (-40.0f)) ? ((0.02424f * exp((-0.01052f) * V)) / (1.0f + exp((-0.1378f) * (V + 40.14f))))
                                 : ((0.6f * exp(0.057f * V)) / (1.0f + exp((-0.1f) * (V + 32.0f))));
    ...
    real tau_f = (1102.5f * exp((-pow(V + 27.0f, 2.0f)) / 225.0f)) + (200.0f / (1.0f + exp((13.0f - V) / 10.0f))) +
                 (180.0f / (1.0f + exp((V + 30.0f) / 10.0f))) + 20.0f;

    real f2_inf = (0.67f / (1.0f + exp((V + 35.0f) / 7.0f))) + 0.33f;
    real tau_f2 = (562.0f * exp((-pow(V + 27.0f, 2.0f)) / 240.0f)) + (31.0f / (1.0f + exp((25.0f - V) / 10.0f))) +
                  (80.0f / (1.0f + exp((V + 30.0f) / 10.0f)));
    ...
    real Ca_i_bufc = 1.0f / (1.0f + ((var_calcium_dynamics__Buf_c * var_calcium_dynamics__K_buf_c) /
                                     pow(Ca_i + var_calcium_dynamics__K_buf_c, 2)));
    ...
    real var_calcium_dynamics__Ca_sr_bufsr =
        1.0f / (1.0f + ((var_calcium_dynamics__Buf_sr * var_calcium_dynamics__K_buf_sr) /
                        pow(Ca_SR + var_calcium_dynamics__K_buf_sr, 2)));
    ...
    real Ca_ss_bufss = 1.0f / (1.0f + ((var_calcium_dynamics__Buf_ss * var_calcium_dynamics__K_buf_ss) /
                                       pow(Ca_ss + var_calcium_dynamics__K_buf_ss, 2.0f)));
