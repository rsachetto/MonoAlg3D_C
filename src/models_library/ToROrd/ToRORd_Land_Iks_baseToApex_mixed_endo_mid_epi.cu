...
extern "C" SET_ODE_INITIAL_CONDITIONS_GPU(set_model_initial_conditions_gpu) {
    ...
    if(solver->ode_extra_data) {
        ...
        check_cuda_error(cudaMalloc((void **)&basetoapex_device, sizeof(real)*num_volumes));
        check_cuda_error(cudaMemcpy(basetoapex_device, basetoapex, sizeof(real)*num_volumes, cudaMemcpyHostToDevice));
    }
    else {
    ...
    if (solver->ode_extra_data) {
        kernel_set_model_initial_conditions_endo_mid_epi<<<GRID, BLOCK_SIZE>>>(solver->sv, num_volumes, pitch_h, use_adpt_dt, solver->min_dt,\
                                                            ...
                                                            transmurality_device, sf_Iks_device, basetoapex_device);
    }
    else {
        log_error_and_exit("[INFO] You should supply a mask function with this mixed model! The mask function must have 'transmurality', 'sf_Iks' and 'basetoapex' fields!\n");
    }
...
extern "C" SOLVE_MODEL_ODES(solve_model_odes_gpu) {
    ...
    if (ode_solver->ode_extra_data) {
        solve_endo_mid_epi_gpu<<<GRID, BLOCK_SIZE>>>(current_t, dt, sv, stims_currents_device, cells_to_solve_device,\
                                                    transmurality_device, sf_Iks_device, basetoapex_device, extra_par_device,\
    ...
    if (basetoapex_device) check_cuda_error(cudaFree(basetoapex_device));
    ...
    if(threadID < num_cells_to_solve) {
        ...
        if(!use_adpt) {
            ...
            for(int n = 0; n < num_steps; ++n) {
                ...
                RHS_RL_gpu(a, b, sv, rDY, stim_currents[threadID], transmurality[threadID], sf_Iks[threadID], basetoapex[threadID], extra_params, sv_id, dt, pitch, false);
            ...
            solve_forward_euler_gpu_adpt(sv, stim_currents[threadID], transmurality[threadID], sf_Iks[threadID], basetoapex[threadID], extra_params, cur_time + max_dt, sv_id, pitch, abstol,  reltol,  dt,  max_dt);
        }
    }
}
...
inline __device__ void solve_forward_euler_gpu_adpt(real *sv, real stim_curr, real transmurality, real sf_Iks, real basetoapex, real *extra_params, real final_time, int thread_id, size_t pitch, real abstol, real reltol, real min_dt, real max_dt) {
    ...
    RHS_gpu(sv_local, rDY, stim_curr, transmurality, sf_Iks, basetoapex, extra_params, thread_id, dt, pitch, true);
    time_new += dt;
