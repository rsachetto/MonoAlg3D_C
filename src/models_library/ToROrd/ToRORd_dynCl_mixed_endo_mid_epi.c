...
SET_ODE_INITIAL_CONDITIONS_CPU(set_model_initial_conditions_cpu) {
    ...
    if(solver->ode_extra_data) {
        struct extra_data_for_torord_cell_wise *extra_data = (struct extra_data_for_torord_cell_wise*)solver->ode_extra_data;
...
real * extract_extra_data(real **extra_par, int i) {
    real *out = malloc(17 * sizeof(real));
    for (int j = 0; j < 17; j++) {
        out[j] = extra_par[j][i];
    }
    return out;
...
SOLVE_MODEL_ODES(solve_model_odes_cpu) {
    ...
    for (u_int32_t i = 0; i < num_cells_to_solve; i++) {
        ...
        real * this_extra_par;
                this_extra_par = extract_extra_data(extra_par, i);
