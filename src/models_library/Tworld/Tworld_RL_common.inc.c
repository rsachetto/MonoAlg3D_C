...
//real d_camk_f_ICaL = (CaMK_Phos_ss_ICaL - camk_f_ICaL) / tau_cal;   
real d_camk_f_ICaL = CaMK_Phos_ss_ICaL-(CaMK_Phos_ss_ICaL-camk_f_ICaL)*exp(-dt/tau_cal);    // Rush-Larsen
//real d_camk_f_RyR = (CaMK_Phos_ss_RyR - camk_f_RyR)  / tau_ryr;     
real d_camk_f_RyR = CaMK_Phos_ss_RyR-(CaMK_Phos_ss_RyR-camk_f_RyR)*exp(-dt/tau_ryr);        // Rush-Larsen
//real d_camk_f_PLB = (CaMK_Phos_ss_PLB - camk_f_PLB)  / tau_plb;     
real d_camk_f_PLB = CaMK_Phos_ss_PLB-(CaMK_Phos_ss_PLB-camk_f_PLB)*exp(-dt/tau_plb);        // Rush-Larsen
...
//real dm = (mss - m) / taum;                     
real dm = mss-(mss-m)*exp(-dt/taum);            // Rush-Larsen
...
//real dh = (hss - h) / tauh;                     
real dh = hss-(hss-h)*exp(-dt/tauh);            // Rush-Larsen
...
//real dj = (jss - j) / tauj;                     
real dj = jss-(jss-j)*exp(-dt/tauj);            // Rush-Larsen

// gating CaMK-P
real hssp = 1 / pow((1 + exp( (v + 71.55 + 6)/7.43 )),2);
//real dhp = (hssp - hp) / tauh;                  
real dhp = hssp-(hssp-hp)*exp(-dt/tauh);      // Rush-Larsen
real taujp = 1.46 * tauj;
//real djp = (jss - jp) / taujp;                  
real djp = jss-(jss-jp)*exp(-dt/taujp);       // Rush-Larsen

// gating PKA
real mss_P = 1 / (pow(1 + exp( -(56.86 + v + 5.0) / 9.03 ),2));

//real dm_P = (mss_P - m_P) / taum;                     
real dm_P = mss_P-(mss_P-m_P)*exp(-dt/taum);  // Rush-Larsen

real hss_P = 1 / (pow(1 + exp( (v + 71.55+5.0)/7.43 ),2));  // BetaAdrenergic;
//real dh_P=(hss_P-h_P)/tauh;
real dh_P = hss_P-(hss_P-h_P)*exp(-dt/tauh);  // Rush-Larsen
real jss_P=hss_P;
//real dj_P = (jss_P - j_P) / tauj;             
real dj_P = jss_P-(jss_P-j_P)*exp(-dt/tauj);  // Rush-Larsen

// Both Phosphorylated
real hssp_P = 1 / (pow(1 + exp( (v + 71.55 + 6 + 5.0)/7.43 ),2)); // BetaAdrenergic;
//real dhp_P = (hssp_P - hp_P) / tauh;                 
real dhp_P = hssp_P-(hssp_P-h_P)*exp(-dt/tauh);      // Rush-Larsen
real jssp_P=hssp_P;
//real djp_P = (jssp_P - jp_P) / taujp;                
real djp_P = jssp_P-(jssp_P-jp_P)*exp(-dt/taujp);    // Rush-Larsen
...
real dmL = mLss-(mLss-mL)*exp(-dt/tmL);     // Rush-Larsen
real hLss=1.0/(1.0+exp((v+87.61)/7.488));
real thL=150.0;
//real dhL=(hLss-hL)/thL;             
real dhL = hLss-(hLss-hL)*exp(-dt/thL);     // Rush-Larsen
real hLssp=1.0/(1.0+exp((v+93.81)/7.488));
real thLp=3.0*thL;
//real dhLp=(hLssp-hLp)/thLp;
real dhLp = hLssp-(hLssp-hLp)*exp(-dt/thLp);    // Rush-Larsen
...
//real d_iks_xs_junc = (xsss_junc-iks_xs_junc)/tauxs_junc;    
real d_iks_xs_junc = xsss_junc-(xsss_junc-iks_xs_junc)*exp(-dt/tauxs_junc);      // Rush-Larsen
//real d_iks_xs_sl = (xsss_sl-iks_xs_sl)/tauxs_sl;            
real d_iks_xs_sl = xsss_sl-(xsss_sl-iks_xs_sl)*exp(-dt/tauxs_sl);                // Rush-Larsen
//     eks = ek;
...
//real d_ito_xtos = (xtoss - ito_xtos)/tauxtos;   
real d_ito_xtos = xtoss-(xtoss-ito_xtos)*exp(-dt/tauxtos);      // Rush-Larsen
//real d_ito_ytos = (ytoss - ito_ytos)/tauytos;   
real d_ito_ytos = ytoss-(ytoss-ito_ytos)*exp(-dt/tauytos);      // Rush-Larsen

real tauxtof = 8.5*exp(-pow((v+45)/50,2))+0.5;
real tauytof = 85*exp((-pow(v+40,2)/220))+7;
//real d_ito_xtof = (xtoss - ito_xtof)/tauxtof;   
real d_ito_xtof = xtoss-(xtoss-ito_xtof)*exp(-dt/tauxtof);      // Rush-Larsen
//real d_ito_ytof = (ytoss - ito_ytof)/tauytof;   
real d_ito_ytof = ytoss-(ytoss - ito_ytof)*exp(-dt/tauytof);    // Rush-Larsen

// CaMKII effect on ss activation
real xtoss_p = 1/(1+exp(-(v-29.0)/13));
//real d_ito_xtos_p = (xtoss_p - ito_xtos_p)/tauxtos; 
real d_ito_xtos_p = xtoss_p-(xtoss_p-ito_xtos_p)*exp(-dt/tauxtos);  // Rush-Larsen
//real d_ito_xtof_p = (xtoss_p - ito_xtof_p)/tauxtof; 
real d_ito_xtof_p = xtoss_p-(xtoss_p-ito_xtof_p)*exp(-dt/tauxtof);  // Rush-Larsen
...
//real d_ito_ytos_p = (ytoss - ito_ytos_p)/tauytos_p; 
real d_ito_ytos_p = ytoss-(ytoss-ito_ytos_p)*exp(-dt/tauytos_p);    // Rush-Larsen
//real d_ito_ytof_p = (ytoss - ito_ytof_p)/tauytof_p; 
real d_ito_ytof_p = ytoss-(ytoss-ito_ytof_p)*exp(-dt/tauytof_p);    // Rush-Larsen
...
if (fICaL_PKA > 0) {
    ...
    //dd = (dss-d)/td;       
    dd = dss-(dss-d)*exp(-dt/td);       // Rush-Larsen
    ...
    Afs=1.0-Aff;
    //dff=(fss-ff)/tff;      
    dff = fss-(fss-ff)*exp(-dt/tff);    // Rush-Larsen
    //dfs=(fss-fs)/tfs;      
    dfs = fss-(fss-fs)*exp(-dt/tfs);    // Rush-Larsen
    f=Aff*ff+Afs*fs;
    fcass=fss;
    ...
    dfcaf = fcass-(fcass-fcaf)*exp(-dt/tfcaf);      // Rush-Larsen
    //dfcas=(fcass-fcas)/tfcas;
    dfcas = fcass-(fcass-fcas)*exp(-dt/tfcas);      // Rush-Larsen
    fca=Afcaf*fcaf+Afcas*fcas;

    tjca = 73.90298;
    jcass = 1.0/(1.0+exp((v+17.66945)/(3.21501)));
    //djca = (jcass-jca)/tjca;   
    djca = jcass-(jcass-jca)*exp(-dt/tjca);         // Rush-Larsen
    tffp=2.5*tff;
    //dffp=(fss-ffp)/tffp;
    dffp = fss-(fss-ffp)*exp(-dt/tffp);             // Rush-Larsen
    fp=Aff*ffp+Afs*fs;
    tfcafp=2.5*tfcaf;
    //dfcafp=(fcass-fcafp)/tfcafp;
    dfcafp = fcass-(fcass-fcafp)*exp(-dt/tfcafp);   // Rush-Larsen
    ...
    dPss=fminf(dPss,1);
    //dd_P =(dPss-d_P)/td;    
    dd_P = dPss-(dPss-d_P)*exp(-dt/td);     // Rush-Larsen
    fss_P=1.0/(1.0+exp((v+19.58+6)/3.696)); //BetaAdrenergic; 
    //dff_P=(fss_P-ff_P)/tff;       
    dff_P = fss_P-(fss_P-ff_P)*exp(-dt/tff);    // Rush-Larsen
    //dfs_P=(fss_P-fs_P)/tfs;       
    dfs_P = fss_P-(fss_P-fs_P)*exp(-dt/tfs);    // Rush-Larsen
    fcass_P=fss_P;
    //dfcaf_P=(fcass_P-fcaf_P)/tfcaf;    
    dfcaf_P = fcass_P-(fcass_P-fcaf_P)*exp(-dt/tfcaf);  // Rush-Larsen
    //dfcas_P=(fcass_P-fcas_P)/tfcas;    
    dfcas_P = fcass_P-(fcass_P-fcas_P)*exp(-dt/tfcas);    // Rush-Larsen
...
//real d_jrel_icaldep_act = (Jrel_inf-jrel_icaldep_act)/tau_rel;  
real d_jrel_icaldep_act = Jrel_inf-(Jrel_inf-jrel_icaldep_act)*exp(-dt/tau_rel);        // Rush-Larsen

// and inactivation
real tauInact = 64.11202;
real Jrel_inact_inf = 1/(1 + (I_Ca_junc_sigmoided/1e-3));
//real d_jrel_icaldep_f1 = (Jrel_inact_inf - jrel_icaldep_f1)/tauInact;   
real d_jrel_icaldep_f1 = Jrel_inact_inf-(Jrel_inact_inf-jrel_icaldep_f1)*exp(-dt/tauInact); // Rush-Larsen

// and slower inactivation
real tauInact2 = 119.48978;
real Jrel_inact_inf2 = 1/(1 + (I_Ca_junc_sigmoided/0.6e-3));
//real d_jrel_icaldep_f2 = (Jrel_inact_inf2 - jrel_icaldep_f2)/tauInact2; 
real d_jrel_icaldep_f2 = Jrel_inact_inf2-(Jrel_inact_inf2-jrel_icaldep_f2)*exp(-dt/tauInact2);  // Rush-Larsen
...
//a_[28] = -1.0/tauh;                 // dhL
//a_[29] = -1.0/tauh;                 // dhLp
//a_[30] = -1.0/tauxtos;              // d_ito_xtos
//a_[31] = -1.0/tauytos;              // d_ito_ytos
//a_[32] = -1.0/tauxtof;              // d_ito_xtof
//a_[33] = -1.0/tauytof;              // d_ito_ytof
//a_[34] = -1.0/tauxtos;              // d_ito_xtos_p
//a_[35] = -1.0/tauytos_p;            // d_ito_ytos_p
//a_[36] = -1.0/tauxtof;              // 
//a_[37] = -1.0/tauytof_p;            // 
//a_[38] = -1.0/tauxs_junc;           // 
//a_[39] = -1.0/tauxs_sl;             // 
//a_[45] = -1.0/td;                   // 
//a_[46] = -1.0/tff;                  // 
//a_[47] = -1.0/tfs;                  // 
...
//b_[28] = hLss/tauh;                 // 
//b_[29] = hLssp/tauh;                // 
//b_[30] = xtoss_p/tauxtos;           // 
//b_[31] = ytoss/tauytos;             // 
//b_[32] = xtoss_p/tauxtof;           // 
//b_[33] = ytoss/tauytof;             // 
//b_[34] = xtoss_p/tauxtos;           // 
//b_[35] = ytoss/tauytos_p;           // 
//b_[36] = xtoss_p/tauxtof;           // 
//b_[37] = ytoss/tauytof_p;           // 
//b_[38] = xsss_junc/tauxs_junc;      // 
//b_[39] = xsss_sl/tauxs_sl;          // 
//b_[45] = dss/td;                    // 
//b_[46] = fss/tff;                   // 
//b_[47] = fss/tfs;                   // 
//b_[48] = fcass/tfcaf;               // 
...
//b_[53] = fss/tffp;                  // 
//b_[54] = fcass/tfcafp;              // 
//b_[65] = Jrel_inf/tau_rel;          // 
//b_[66] = Jrel_inact_inf/tauInact;   // 
//b_[67] = Jrel_inact_inf2/tauInact2; // 
//b_[75] = CaMK_Phos_ss_ICaL/tau_cal; // 
//b_[76] = CaMK_Phos_ss_RyR/tau_ryr;  // 
//b_[77] = CaMK_Phos_ss_PLB/tau_plb;  // 
//b_[79] = hssp/tauh;                 // 
//b_[80] = jss/taujp;                 // 
//b_[81] = mss_P/taum;                // 
//b_[82] = hss_P/tauh;                // 
