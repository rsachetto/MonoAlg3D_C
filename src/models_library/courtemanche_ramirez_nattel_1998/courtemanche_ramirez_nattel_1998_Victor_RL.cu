...
__global__ void kernel_set_model_initial_conditions(real *sv, int num_volumes, size_t pitch, bool use_adpt_dt, real min_dt, real *extra_parameters) {
    ...
    if (threadID < num_volumes) {
        ...
        if (extra_parameters == NULL) {
            ...
            *((real * )((char *) sv + pitch * 0) + threadID)  = extra_parameters[13];
            *((real * )((char *) sv + pitch * 1) + threadID)  = extra_parameters[14];
            *((real * )((char *) sv + pitch * 2) + threadID)  = extra_parameters[15];
            *((real * )((char *) sv + pitch * 3) + threadID)  = extra_parameters[16];
            *((real * )((char *) sv + pitch * 4) + threadID)  = extra_parameters[17];
            *((real * )((char *) sv + pitch * 5) + threadID)  = extra_parameters[18];
            *((real * )((char *) sv + pitch * 6) + threadID)  = extra_parameters[19];
            *((real * )((char *) sv + pitch * 7) + threadID)  = extra_parameters[20];
            *((real * )((char *) sv + pitch * 8) + threadID)  = extra_parameters[21];
            *((real * )((char *) sv + pitch * 9) + threadID)  = extra_parameters[22];
            *((real * )((char *) sv + pitch * 10) + threadID) = extra_parameters[23];
            *((real * )((char *) sv + pitch * 11) + threadID) = extra_parameters[24];
            *((real * )((char *) sv + pitch * 12) + threadID) = extra_parameters[25];
            *((real * )((char *) sv + pitch * 13) + threadID) = extra_parameters[26];
            *((real * )((char *) sv + pitch * 14) + threadID) = extra_parameters[27];
            *((real * )((char *) sv + pitch * 15) + threadID) = extra_parameters[28];
            *((real * )((char *) sv + pitch * 16) + threadID) = extra_parameters[29];
            *((real * )((char *) sv + pitch * 17) + threadID) = extra_parameters[30];
            *((real * )((char *) sv + pitch * 18) + threadID) = extra_parameters[31];
            *((real * )((char *) sv + pitch * 19) + threadID) = extra_parameters[32];
            *((real * )((char *) sv + pitch * 20) + threadID) = extra_parameters[33];
        }
        if(use_adpt_dt) {
...
inline __device__ void RHS_gpu(real *sv, real *rDY, real stim_current, int thread_id, real dt, size_t pitch, bool use_adpt_dt, real *extra_parameters, int mapping) {
...
extern "C" SET_ODE_INITIAL_CONDITIONS_GPU(set_model_initial_conditions_gpu) {
    ...
    kernel_set_model_initial_conditions<<<GRID, BLOCK_SIZE>>>(solver->sv, num_volumes, pitch_h, use_adpt_dt, solver->min_dt, extra_parameters_device);
...
inline __device__ void solve_forward_euler_gpu_adpt(real *sv, real stim_curr, real final_time, int thread_id, size_t pitch, real abstol, real reltol, real min_dt, real max_dt, real *extra_parameters, int mapping) {
    ...
    RHS_gpu(sv_local, rDY, stim_curr, thread_id, dt, pitch, true, extra_parameters, mapping);
    time_new += dt;
...
__global__ void solve_gpu(real cur_time, real dt, real *sv, real *stim_currents, uint32_t *cells_to_solve,
                          ...
                          real abstol, real reltol, real max_dt, real *extra_parameters) {
    int threadID = blockDim.x * blockIdx.x + threadIdx.x;
    int sv_id;
    ...
    if(threadID < num_cells_to_solve) {
        ...
        if(!use_adpt) {
            ...
            for(int n = 0; n < num_steps; ++n) {
                ...
                RHS_gpu(sv, rDY, stim_currents[threadID], sv_id, dt, pitch, false, extra_parameters, mapping);
            ...
            solve_forward_euler_gpu_adpt(sv, stim_currents[threadID], cur_time + max_dt, sv_id, pitch, abstol,  reltol,  dt,  max_dt, extra_parameters, mapping);
        }
    }
}
