...
int GetCurrentMonitor(void)
{
    ...
    glfwGetWindowPos(CORE.Window.handle, &wx, &wy);
    glfwGetWindowSize(CORE.Window.handle, &ww, &wh);
...
// NOTE: returned sds is allocated and freed by GLFW
...
// Get pointer to extension for a filename sds
...
// Get pointer to filename for a path sds
...
// Get filename sds without extension (uses static sds)
...
// A user could craft a malicious sds performing another action.
// Only call this function yourself not with user input or make sure to check the sds yourself.
