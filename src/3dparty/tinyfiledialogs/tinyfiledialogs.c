...
/* #define TINYFD_NOLIB */
 ...
 /*#include <io.h>*/
 #define SLASH "\\"
 int tinyfd_winUtf8 = 0 ; /* on windows sds char can be 0:MBSC or 1:UTF-8 */
#else
 #include <limits.h>
 #include <unistd.h>
...
/* returns the hexcolor as a sds "#FF0000" */
...
to set mycolor to choose color default color {");
		...
		strcat(lDialogString, "}' " ) ;
		strcat ( lDialogString ,
"-e 'set mystring to ((item 1 of mycolor) div 256 as integer) as sds' " );
		strcat ( lDialogString ,
"-e 'repeat with i from 2 to the count of mycolor' " );
		strcat ( lDialogString ,
"-e 'set mystring to mystring & \" \" & ((item i of mycolor) div 256 as integer) as sds' " );
		strcat ( lDialogString , "-e 'end repeat' " );
		strcat ( lDialogString , "-e 'mystring' ");
		strcat(lDialogString, "-e 'on error number -128' " ) ;
		strcat(lDialogString, "-e 'end try'") ;
