...
def main ():
    ...
    if ( len(sys.argv) != 4 ):
        print("-------------------------------------------------------------------------")
        print("Usage:> python %s <num_cells> <num_aps> <ms_each_step>" % sys.argv[0])
        print("-------------------------------------------------------------------------")
        print("<num_cells> = Number of cells that APD must be calculated")
    ...
    # First we get the indexes of the cells we going to calculate the APD
    cell_ids = []
    for i in range(num_cells):
        cell_id = raw_input("Cell id number %d: " % i)
    ...
    for i in range(num_cells):
        ...
        ap_file = open("inputs/ap-%d.txt" % cell_ids[i])
        ...
        for j in range(num_aps):
            ...
            slope_s = slope_start(ap_data,slope_e,0.0001,ms_each_step)
            print("[Cell %d] Slope start = %g ms\n" % (i,slope_s))   

            max_ind = max_index(ap_data, slope_e, 1050)
            print("[Cell %d] Max ind = %d\n" % (i,max_ind))

            slope_e = slope_end(ap_data, max_ind, 0.0005, ms_each_step)
            print("[Cell %d] Slope end = %g\n" % (i,slope_e))
            print("**************************************************************")   
