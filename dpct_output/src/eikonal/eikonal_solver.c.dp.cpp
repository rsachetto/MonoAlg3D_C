...
void free_eikonal_solver(struct eikonal_solver *solver) {
    ...
    if(solver->cuda_mem_created) {
        ...
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->memory_struct.d_spd, dpct::get_in_order_queue())));
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->memory_struct.d_sol, dpct::get_in_order_queue())));
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->memory_struct.t_sol, dpct::get_in_order_queue()))); // temp solution for ping-pong
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->memory_struct.d_con, dpct::get_in_order_queue()))); // convergence volume
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->memory_struct.d_list, dpct::get_in_order_queue())));
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->memory_struct.d_listVol, dpct::get_in_order_queue())));
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->memory_struct.d_mask, dpct::get_in_order_queue())));
    }
...
void check_cuda_memory(struct eikonal_solver *solver) {
    if (solver->verbose) {
        ...
        /*
        DPCT1106:15: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may not be supported by all compilers or runtimes. You
...
void init_cuda_mem(struct eikonal_solver *solver) {
    ...
    //
    check_cuda_error(DPCT_CHECK_ERROR(solver->memory_struct.d_spd = sycl::malloc_device<real>(volSize, dpct::get_in_order_queue())));
    check_cuda_memory(solver);

    check_cuda_error(DPCT_CHECK_ERROR(solver->memory_struct.d_sol = sycl::malloc_device<real>(volSize, dpct::get_in_order_queue())));
    check_cuda_memory(solver);

    check_cuda_error(
        DPCT_CHECK_ERROR(solver->memory_struct.t_sol = sycl::malloc_device<real>(volSize, dpct::get_in_order_queue()))); // temp solution for ping-pong
    check_cuda_memory(solver);

    check_cuda_error(
        DPCT_CHECK_ERROR(solver->memory_struct.d_con = (_Bool *)sycl::malloc_device(volSize * sizeof(bool), dpct::get_in_order_queue()))); // convergence volume
    check_cuda_memory(solver);

    check_cuda_error(DPCT_CHECK_ERROR(solver->memory_struct.d_list = sycl::malloc_device<uint>(blockNum, dpct::get_in_order_queue())));
    check_cuda_memory(solver);

    check_cuda_error(DPCT_CHECK_ERROR(solver->memory_struct.d_listVol = (_Bool *)sycl::malloc_device(blockNum * sizeof(bool), dpct::get_in_order_queue())));
    check_cuda_memory(solver);

    check_cuda_error(DPCT_CHECK_ERROR(solver->memory_struct.d_mask = (_Bool *)sycl::malloc_device(volSize * sizeof(bool), dpct::get_in_order_queue())));
    check_cuda_memory(solver);
}
...
void set_attribute_mask(struct eikonal_solver *solver) {
    ...
    check_cuda_error(DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(solver->memory_struct.d_spd, h_spd, volSize * sizeof(real)).wait()));
    check_cuda_error(DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(solver->memory_struct.d_mask, h_mask, volSize * sizeof(bool)).wait()));
...
static void get_solution(struct eikonal_solver *solver) {
    ...
    check_cuda_error(DPCT_CHECK_ERROR(
        dpct::get_in_order_queue().memcpy(solver->memory_struct.h_sol, solver->memory_struct.d_sol, solver->memory_struct.volsize * sizeof(real)).wait()));
...
void use_seeds(struct eikonal_solver *solver) {
    ...
    // initialize GPU memory with host memory
    check_cuda_error(
        DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(solver->memory_struct.d_sol, solver->memory_struct.h_sol, volSize * sizeof(real)).wait()));
    check_cuda_error(
        DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(solver->memory_struct.t_sol, solver->memory_struct.h_sol, volSize * sizeof(real)).wait()));
    check_cuda_error(
        DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(solver->memory_struct.d_list, solver->memory_struct.h_list, nActiveBlock * sizeof(uint)).wait()));
    check_cuda_error(
        DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(solver->memory_struct.d_listVol, solver->memory_struct.h_listVol, blockNum * sizeof(bool)).wait()));
    // initialize GPU memory with constant value
    check_cuda_error(DPCT_CHECK_ERROR(dpct::get_in_order_queue().memset(solver->memory_struct.d_con, 1, volSize * sizeof(bool)).wait()));
}
