...
SAVE_MESH(save_one_cell_state_variables) {
    ...
    if(ode_solver->gpu) {
        ...
        check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_memcpy(cell_sv, sizeof(real), ode_solver->sv + params->cell_sv_position, ode_solver->pitch, sizeof(real),
            ...
            if(binary) {
                ...
                if(save_ode_state_variables) {
                    ...
                    if(ode_solver->gpu) {
                        ...
                        check_cuda_error(
                            DPCT_CHECK_ERROR(dpct::dpct_memcpy(sv_cpu, ode_solver->original_num_cells * sizeof(real), ode_solver->sv, ode_solver->pitch,
...
#ifdef COMPILE_CUDA
            sv_cpu = MALLOC_ARRAY_OF_TYPE(real, ode_solver->original_num_cells * num_sv_entries);
            check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_memcpy(sv_cpu, ode_solver->original_num_cells * sizeof(real), ode_solver->sv, ode_solver->pitch,
                                                                ode_solver->original_num_cells * sizeof(real), num_sv_entries, cudaMemcpyDeviceToHost)));
#endif
        } else {
            sv_cpu = ode_solver->sv;
...
SAVE_MESH(save_multiple_cell_state_variables) {
    ...
    if(ode_solver->gpu) {
        ...
        for (uint32_t k = 0; k < params->num_cells; k++) {
            ...
            check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_memcpy(cell_sv, sizeof(real), ode_solver->sv + params->cell_sv_positions[k], ode_solver->pitch,
