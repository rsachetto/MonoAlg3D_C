...
INIT_CALC_ECG(init_pseudo_bidomain_gpu) {
    ...
    check_cuda_error(
        DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->vec_beta_im = std::make_shared<dpct::sparse::dense_vector_desc>(N, PSEUDO_BIDOMAIN_DATA->beta_im, BLAS_SIZE)));
    check_cuda_error(DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->vec_vm = std::make_shared<dpct::sparse::dense_vector_desc>(N, the_ode_solver->sv, BLAS_SIZE)));
    ...
    check_cuda_error(DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->bufferSize = 0));
...
CALC_ECG(pseudo_bidomain_gpu) {
    ...
    check_cublas_error(
        DPCT_CHECK_ERROR(dpct::sparse::spmv((PSEUDO_BIDOMAIN_DATA->sparseHandle)->get_queue(), oneapi::mkl::transpose::nontrans, &alpha,
                                            PSEUDO_BIDOMAIN_DATA->matA, PSEUDO_BIDOMAIN_DATA->vec_vm, &beta, PSEUDO_BIDOMAIN_DATA->vec_beta_im, BLAS_SIZE)));
#else
...
#ifdef CELL_MODEL_REAL_DOUBLE
        /*
        DPCT1034:13: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this code.
        */
        check_cublas_error([&]() {
        dpct::blas::wrapper_double_out res_wrapper_ct6((PSEUDO_BIDOMAIN_DATA->blasHandle)->get_queue(), &local_sum);
        oneapi::mkl::blas::column_major::dot((PSEUDO_BIDOMAIN_DATA->blasHandle)->get_queue(), n_active, PSEUDO_BIDOMAIN_DATA->tmp_data, 1,
                                             PSEUDO_BIDOMAIN_DATA->d_volumes, 1, res_wrapper_ct6.get_ptr());
        return 0;
        }());
#else
        check_cublas_error(
...
END_CALC_ECG(end_pseudo_bidomain_gpu) {
    ...
    check_cuda_error((dpct::err0)DPCT_CHECK_ERROR(delete(persistent_data->sparseHandle)));
    check_cuda_error((dpct::err0)DPCT_CHECK_ERROR(delete(persistent_data->blasHandle)));

#if CUBLAS_VER_MAJOR >= 11
    if(persistent_data->matA) {
        check_cuda_error(DPCT_CHECK_ERROR((persistent_data->matA).reset()));
    }
    if(persistent_data->vec_beta_im) {
        check_cuda_error(DPCT_CHECK_ERROR((persistent_data->vec_beta_im).reset()));
    }
    if(persistent_data->vec_vm) {
        check_cuda_error(DPCT_CHECK_ERROR((persistent_data->vec_vm).reset()));
    }
#else
...
INIT_CALC_ECG(init_pseudo_bidomain_with_diffusive_current_gpu) {
    ...
    check_cublas_error(DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->sparseHandle = new dpct::sparse::descriptor()));
    check_cublas_error(DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->blasHandle = new dpct::blas::descriptor()));
...
#if CUBLAS_VER_MAJOR >= 11
    check_cuda_error(
        DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->matA = std::make_shared<dpct::sparse::sparse_matrix_desc>(
                             N, N, nz, PSEUDO_BIDOMAIN_DATA->d_row, PSEUDO_BIDOMAIN_DATA->d_col, PSEUDO_BIDOMAIN_DATA->d_val, dpct::library_data_t::real_int32,
                             dpct::library_data_t::real_int32, oneapi::mkl::index_base::zero, BLAS_SIZE, dpct::sparse::matrix_format::csr)));
    check_cuda_error(
        DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->vec_beta_im = std::make_shared<dpct::sparse::dense_vector_desc>(N, PSEUDO_BIDOMAIN_DATA->beta_im, BLAS_SIZE)));
    check_cuda_error(DPCT_CHECK_ERROR(PSEUDO_BIDOMAIN_DATA->vec_vm = std::make_shared<dpct::sparse::dense_vector_desc>(N, the_ode_solver->sv, BLAS_SIZE)));
#else
...
CALC_ECG(pseudo_bidomain_with_diffusive_current_gpu) {
    ...
    if (time_info->iteration % PSEUDO_BIDOMAIN_DATA->diff_curr_rate == 0 && time_info->current_t <= PSEUDO_BIDOMAIN_DATA->diff_curr_max_time) {
        ...
        check_cuda_error(DPCT_CHECK_ERROR(
            dpct::get_in_order_queue().memcpy(PSEUDO_BIDOMAIN_DATA->beta_im_cpu, PSEUDO_BIDOMAIN_DATA->beta_im, n_active * sizeof(real)).wait()));
        for(uint32_t i = 0; i < n_active; i++) {
...
#ifdef CELL_MODEL_REAL_DOUBLE
        /*
        DPCT1034:14: Migrated API does not return an error code. 0 is returned in the lambda. You may need to rewrite this code.
        */
        check_cublas_error([&]() {
        dpct::blas::wrapper_double_out res_wrapper_ct6((PSEUDO_BIDOMAIN_DATA->blasHandle)->get_queue(), &local_sum);
        oneapi::mkl::blas::column_major::dot((PSEUDO_BIDOMAIN_DATA->blasHandle)->get_queue(), n_active, PSEUDO_BIDOMAIN_DATA->tmp_data, 1,
