...
static const char * cudaGetErrorEnum(int code, const char *api) {
	if(strcmp(api, "cuda") == 0) {
        dpct::err0 error = (dpct::err0)code;
        /*
        DPCT1009:1: SYCL reports errors using exceptions and does not use error codes. Please replace the "get_error_string_dummy(...)" with a real
