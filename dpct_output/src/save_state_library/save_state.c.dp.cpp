...
SAVE_STATE(save_simulation_state) {
    ...
    if(the_ode_solver) {
        ...
        if(the_ode_solver->gpu) {
            ...
            check_cuda_error(
                DPCT_CHECK_ERROR(dpct::dpct_memcpy(sv_cpu, the_ode_solver->original_num_cells * sizeof(real), the_ode_solver->sv, the_ode_solver->pitch,
...
SAVE_STATE(save_purkinje_coupled_simulation_state) {
    ...
    // PURKINJE section
        ...
        if(the_purkinje_ode_solver->gpu) {
            ...
            check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_memcpy(sv_cpu, the_purkinje_ode_solver->original_num_cells * sizeof(real), the_purkinje_ode_solver->sv,
                                                                the_purkinje_ode_solver->pitch, the_purkinje_ode_solver->original_num_cells * sizeof(real),
                                                                num_sv_entries, cudaMemcpyDeviceToHost)));
