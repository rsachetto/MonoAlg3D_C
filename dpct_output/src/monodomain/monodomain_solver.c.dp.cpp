...
int solve_monodomain(struct monodomain_solver *the_monodomain_solver, struct ode_solver *the_ode_solver, struct grid *the_grid, struct user_options *configs,
    ...
    if(init_gpu) {
        ...
        check_cuda_error(DPCT_CHECK_ERROR(device_count = dpct::device_count()));
        ...
        if(device_count > 0) {
            ...
            check_cuda_error(DPCT_CHECK_ERROR(dpct::get_device(the_ode_solver->gpu_id).get_device_info(prop)));
            ...
            /*
            DPCT1093:2: The "device" device may be not the one intended for use. Adjust the selected device if needed.
            */
            check_cuda_error(DPCT_CHECK_ERROR(dpct::select_device(device)));
        }
    }
#endif
...
bool update_ode_state_vector_and_check_for_activity(real_cpu vm_threshold, struct ode_solver *the_ode_solver, struct ode_solver *the_purkinje_ode_solver,
    ...
    if(the_ode_solver) {
        ...
        if(the_ode_solver->gpu) {
            ...
            check_cuda_error(DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(sv, vms, mem_size).wait()));
            free(vms);
#endif
        } else {
            OMP(parallel for)
    ...
    if(the_purkinje_ode_solver) {
        ...
        if(the_purkinje_ode_solver->gpu) {
            ...
            if(the_grid->adaptive)
                check_cuda_error(DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(vms_purkinje, sv_purkinje, mem_size_purkinje).wait()));
            ...
            check_cuda_error(DPCT_CHECK_ERROR(dpct::get_in_order_queue().memcpy(sv_purkinje, vms_purkinje, mem_size_purkinje).wait()));
            free(vms_purkinje);
#endif
        } else {
            OMP(parallel for)
