...
void RHS_cpu(const real *sv, real *rDY_, real stim_current, real dt) {
    ...
    real calc_alpha = ((0.4f * exp(((V_old_ + 12.0f) / 10.0f)) *
                        ((1.0f + (0.7f * exp(((-pow((V_old_ + 40.0f), 2.0f)) / 10.0f)))) - (0.75f * exp(((-pow((V_old_ + 20.0f), 2.0f)) / 400.0f))))) /
                       (1.0f + (0.12f * exp(((V_old_ + 12.0f) / 10.0f))))); // 31
    ...
    real calc_i_NaCa =
        (((((((k_NaCa * 1.0f) / (pow(K_mNa, 3.0) + pow(Nao, 3.0))) * 1.0f) / (K_mCa + Cao)) * 1.0f) /
          (1.0f + (k_sat * exp((((eta - 1.0f) * V_old_ * F) / (R * T)))))) *
         ((exp(((eta * V_old_ * F) / (R * T))) * pow(Nai_old_, 3.0) * Cao) - (exp((((eta - 1.0f) * V_old_ * F) / (R * T))) * pow(Nao, 3.0) * Cai_old_))); // 36
    ...
    real d_dt_P_O1 = (((k_plus_a * pow(Cass_old_, n) * calc_P_C1) + (k_minus_b * P_O2_old_) + (k_minus_c * P_C2_old_)) -
                      ((k_minus_a * P_O1_old_) + (k_plus_b * pow(Cass_old_, m) * P_O1_old_) + (k_plus_c * P_O1_old_))); // 18
