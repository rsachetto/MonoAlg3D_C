...
void RHS_cpu(const real *sv, real *rDY_, real dt, real stim_current) {
    ...
    real num =
        inacamax * (pow(nasss_old, 3) * cao * exp(nu * v_old * frdy / (R * temp)) - pow(nao, 3) * 1.5 * casss_old * exp((nu - 1) * v_old * frdy / (R * temp)));
    ...
    num =
        inacamax * (pow(nassl_old, 3) * cao * exp(nu * v_old * frdy / (R * temp)) - pow(nao, 3) * 1.5 * cassl_old * exp((nu - 1) * v_old * frdy / (R * temp)));
    ...
    cassl_old = (2.0 / 3.0) * sqrt(bmyo * bmyo - 3.0 * cmyo) *
                    cos(acos((9.0 * bmyo * cmyo - 2 * bmyo * bmyo * bmyo - 27 * dmyo) / (2.0 * pow((bmyo * bmyo - 3.0 * cmyo), 1.5))) / 3.0) -
                bmyo / 3.0;
    ...
    cai_old = (2.0 / 3.0) * sqrt(bmyo * bmyo - 3.0 * cmyo) *
                  cos(acos((9.0 * bmyo * cmyo - 2 * bmyo * bmyo * bmyo - 27 * dmyo) / (2.0 * pow((bmyo * bmyo - 3.0 * cmyo), 1.5))) / 3.0) -
              bmyo / 3.0;
