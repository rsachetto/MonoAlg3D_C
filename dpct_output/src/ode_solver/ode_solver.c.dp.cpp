...
void free_ode_solver(struct ode_solver *solver) {
    if(solver->sv) {
        if(solver->gpu) {
#ifdef COMPILE_CUDA
            dpct::dpct_free(solver->sv, dpct::get_in_order_queue());
#endif
        } else {
            free(solver->sv);
        }
...
void set_ode_initial_conditions_for_all_volumes(struct ode_solver *solver, struct string_hash_entry *ode_extra_config) {
    ...
    if(solver->gpu) {
        ...
        if(solver->sv != NULL) {
            check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_free(solver->sv, dpct::get_in_order_queue())));
        }
    ...
    if(ode_solver->gpu) {
        ...
        for(i = 0; i < num_refined_cells; i++) {
            ...
            for(int j = 1; j < max_index; j++) {
                ...
                sv_dst = &sv[index];
                check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_memcpy(sv_dst, pitch_h, sv_src, pitch_h, sizeof(real), (size_t)neq, cudaMemcpyDeviceToDevice)));
            }
        }
#endif
    } else {
