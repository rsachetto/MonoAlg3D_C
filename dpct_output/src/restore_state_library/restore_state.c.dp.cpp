...
RESTORE_STATE (restore_simulation_state) {
    ...
    if (the_ode_solver) {
        ...
        if(read_cells_to_solve) {
            the_ode_solver->cells_to_solve = (uint32_t *)malloc(sizeof(the_ode_solver->cells_to_solve[0]) * the_ode_solver->num_cells_to_solve);
        ...
        if (the_ode_solver->gpu) {
            ...
            check_cuda_error(
                DPCT_CHECK_ERROR(dpct::dpct_memcpy(the_ode_solver->sv, the_ode_solver->pitch, sv_cpu, the_ode_solver->original_num_cells * sizeof(real),
...
RESTORE_STATE (restore_purkinje_coupled_simulation_state) {
    ...
    // PURKINJE section
        ...
        if(read_cells_to_solve) {
            the_purkinje_ode_solver->cells_to_solve =
                (uint32_t *)malloc(sizeof(the_purkinje_ode_solver->cells_to_solve[0]) * the_purkinje_ode_solver->num_cells_to_solve);
        ...
        if (the_purkinje_ode_solver->gpu) {
            ...
            check_cuda_error(DPCT_CHECK_ERROR(dpct::dpct_memcpy(
                the_purkinje_ode_solver->sv, the_purkinje_ode_solver->pitch, sv_cpu, the_purkinje_ode_solver->original_num_cells * sizeof(real),
