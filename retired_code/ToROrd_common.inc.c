 //Parameters
    ...
    real calc_CaMKb = ((CaMKo*(1.0f-CaMKt_old_))/(1.0f+(KmCaM/cass_old_)));	//11
    real calc_cmdnmax = 0.0f;
    IFNUMBER_1(calc_cmdnmax);	//14
    real calc_Bcass = (1.0f/(1.0f+((BSRmax*KmBSR)/powf((KmBSR+cass_old_),2.000000000000000e+00f))+((BSLmax*KmBSL)/powf((KmBSL+cass_old_),2.000000000000000e+00f))));	//21
    real calc_Bcajsr = (1.0f/(1.0f+((csqnmax*kmcsqn)/powf((kmcsqn+cajsr_old_),2.000000000000000e+00f))));	//24
    ...
    real calc_bkik = (1.0f/(1.0f+powf((A_atp/K_atp),2.000000000000000e+00f)));	//31
    real calc_mss = (1.0f/powf((1.0f+expf(((-(v_old_+5.686000000000000e+01f))/9.029999999999999e+00f))),2.000000000000000e+00f));	//33
    ...
    real calc_hss = (1.0f/powf((1.0f+expf(((v_old_+7.155000000000000e+01f)/7.430000000000000e+00f))),2.000000000000000e+00f));	//36
    real calc_ah = 0.0f;
    ...
    real calc_hssp = (1.0f/powf((1.0f+expf(((v_old_+7.755000000000000e+01f)/7.430000000000000e+00f))),2.000000000000000e+00f));	//46
    real calc_mLss = (1.0f/(1.0f+expf(((-(v_old_+4.285000000000000e+01f))/5.264000000000000e+00f))));	//52
    ...
    real calc_hLss = (1.0f/(1.0f+expf(((v_old_+8.761000000000000e+01f)/7.488000000000000e+00f))));	//55
    real calc_hLssp = (1.0f/(1.0f+expf(((v_old_+9.381000000000000e+01f)/7.488000000000000e+00f))));	//57
    real calc_thLp = (3.000000000000000e+00f*thL);	//58
    real calc_GNaL = 0.0f;
    IFNUMBER_6(calc_GNaL);	//60
    real calc_ass = (1.0f/(1.0f+expf(((-((v_old_+EKshift)-1.434000000000000e+01f))/1.482000000000000e+01f))));	//63
    real calc_ta = (1.051500000000000e+00f/((1.0f/(1.208900000000000e+00f*(1.0f+expf(((-((v_old_+EKshift)-1.840990000000000e+01f))/2.938140000000000e+01f)))))+(3.500000000000000e+00f/(1.0f+expf(((v_old_+EKshift+1.000000000000000e+02f)/2.938140000000000e+01f))))));	//64
    real calc_iss = (1.0f/(1.0f+expf(((v_old_+EKshift+4.394000000000000e+01f)/5.711000000000000e+00f))));	//66
    real calc_delta_epi = 0.0f;
    IFNUMBER_7(calc_delta_epi);	//67
    real calc_tiF_b = (4.562000000000000e+00f+(1.0f/((3.933000000000000e-01f*expf(((-(v_old_+EKshift+1.000000000000000e+02f))/1.000000000000000e+02f)))+(8.004000000000000e-02f*expf(((v_old_+EKshift+5.000000000000000e+01f)/1.659000000000000e+01f))))));	//68
    real calc_tiS_b = (2.362000000000000e+01f+(1.0f/((1.416000000000000e-03f*expf(((-(v_old_+EKshift+9.652000000000000e+01f))/5.905000000000000e+01f)))+(1.780000000000000e+00f*expf(((v_old_+EKshift+1.141000000000000e+02f)/8.079000000000001e+00f))))));	//69
    real calc_AiF = (1.0f/(1.0f+expf((((v_old_+EKshift)-2.136000000000000e+02f)/1.512000000000000e+02f))));	//72
    real calc_assp = (1.0f/(1.0f+expf(((-((v_old_+EKshift)-2.434000000000000e+01f))/1.482000000000000e+01f))));	//77
    real calc_dti_develop = (1.354000000000000e+00f+(1.0f/(expf((((v_old_+EKshift)-1.674000000000000e+02f)/1.589000000000000e+01f))+expf(((-((v_old_+EKshift)-1.223000000000000e+01f))/2.154000000000000e-01f)))));	//79
    real calc_dti_recover = (1.0f-(5.000000000000000e-01f/(1.0f+expf(((v_old_+EKshift+7.000000000000000e+01f)/2.000000000000000e+01f)))));	//80
    real calc_Gto = 0.0f;
    IFNUMBER_8(calc_Gto);	//86
    real calc_dss = 0.0f;
    IFNUMBER_9(calc_dss);	//89
    real calc_td = (offset+6.000000000000000e-01f+(1.0f/(expf(((-5.000000000000000e-02f)*(v_old_+vShift+6.000000000000000e+00f)))+expf((9.000000000000000e-02f*(v_old_+vShift+1.400000000000000e+01f))))));	//90
    real calc_fss = (1.0f/(1.0f+expf(((v_old_+1.958000000000000e+01f)/3.696000000000000e+00f))));	//92
    real calc_tff = (7.000000000000000e+00f+(1.0f/((4.500000000000000e-03f*expf(((-(v_old_+2.000000000000000e+01f))/1.000000000000000e+01f)))+(4.500000000000000e-03f*expf(((v_old_+2.000000000000000e+01f)/1.000000000000000e+01f))))));	//93
    real calc_tfs = (1.000000000000000e+03f+(1.0f/((3.500000000000000e-05f*expf(((-(v_old_+5.000000000000000e+00f))/4.000000000000000e+00f)))+(3.500000000000000e-05f*expf(((v_old_+5.000000000000000e+00f)/6.000000000000000e+00f))))));	//94
    real calc_Afs = (1.0f-Aff);	//95
    real calc_tfcaf = (7.000000000000000e+00f+(1.0f/((4.000000000000000e-02f*expf(((-(v_old_-4.000000000000000e+00f))/7.000000000000000e+00f)))+(4.000000000000000e-02f*expf(((v_old_-4.000000000000000e+00f)/7.000000000000000e+00f))))));	//100
    real calc_tfcas = (1.000000000000000e+02f+(1.0f/((1.200000000000000e-04f*expf(((-v_old_)/3.000000000000000e+00f)))+(1.200000000000000e-04f*expf((v_old_/7.000000000000000e+00f))))));	//101
    real calc_Afcaf = (3.000000000000000e-01f+(6.000000000000000e-01f/(1.0f+expf(((v_old_-1.000000000000000e+01f)/1.000000000000000e+01f)))));	//102
    real calc_jcass = (1.0f/(1.0f+expf(((v_old_+1.808000000000000e+01f)/2.791600000000000e+00f))));	//107
    real calc_km2n = (jca_old_*1.0f);	//115
    ...
    real calc_constA = (1.826000000000000e+00f*powf((dielConstant*T),(-1.500000000000000e+00f)));	//120
    real calc_PCa = 0.0f;
    ...
    real calc_xs1ss = (1.0f/(1.0f+expf(((-(v_old_+1.160000000000000e+01f))/8.932000000000000e+00f))));	//170
    real calc_txs1 = (8.173000000000000e+02f+(1.0f/((2.326000000000000e+00f*expf(((v_old_+4.828000000000000e+01f)/1.780000000000000e+01f)))+(1.292000000000000e-03f*expf(((-(v_old_+2.100000000000000e+02f))/2.300000000000000e+02f))))));	//171
    real calc_txs2 = (1.0f/((1.000000000000000e-02f*expf(((v_old_-5.000000000000000e+01f)/2.000000000000000e+01f)))+(1.930000000000000e-02f*expf(((-(v_old_+6.654000000000001e+01f))/3.100000000000000e+01f)))));	//174
    real calc_KsCa = (1.0f+(6.000000000000000e-01f/(1.0f+powf((3.800000000000000e+00f/cai_old_),1.400000000000000e+00f))));	//176
    real calc_GKs = 0.0f;
    IFNUMBER_12(calc_GKs);	//177
    real calc_GK1 = 0.0f;
    IFNUMBER_13(calc_GK1);	//182
    real calc_h4_i = (1.0f+((nai_old_/kna1)*(1.0f+(nai_old_/kna2))));	//189
    real calc_h10_i = (kasymm+1.0f+((nao/kna1)*(1.0f+(nao/kna2))));	//195
    real calc_k2_i = kcaoff;	//199
    real calc_k5_i = kcaoff;	//206
    real calc_allo_i = (1.0f/(1.0f+powf((KmCaAct/cai_old_),2.000000000000000e+00f)));	//218
    real calc_Gncx = 0.0f;
    IFNUMBER_14(calc_Gncx);	//221
    real calc_h4_ss = (1.0f+((nass_old_/kna1)*(1.0f+(nass_old_/kna2))));	//226
    real calc_h10_ss = (kasymm+1.0f+((nao/kna1)*(1.0f+(nao/kna2))));	//232
    real calc_k2_ss = kcaoff;	//236
    real calc_k5_ss = kcaoff;	//243
    real calc_allo_ss = (1.0f/(1.0f+powf((KmCaAct/cass_old_),2.000000000000000e+00f)));	//255
    real calc_P = (eP/(1.0f+(H/Khp)+(nai_old_/Knap)+(ki_old_/Kxkur)));	//261
    real calc_b1 = (k1m*MgADP);	//263
    real calc_a2 = k2p;	//264
    real calc_a4 = (((k4p*MgATP)/Kmgatp)/(1.0f+(MgATP/Kmgatp)));	//268
    real calc_Pnak = 0.0f;
    IFNUMBER_15(calc_Pnak);	//280
    real calc_xkb = (1.0f/(1.0f+expf(((-(v_old_-1.089680000000000e+01f))/2.398710000000000e+01f))));	//282
    real calc_GKb = 0.0f;
    ...
    real calc_a_rel = ((5.000000000000000e-01f*bt)/1.0f);	//295
    real calc_tau_rel_b = (bt/(1.0f+(1.230000000000000e-02f/cajsr_old_)));	//298
    ...
    real calc_Bcai = (1.0f/(1.0f+((calc_cmdnmax*kmcmdn)/powf((kmcmdn+cai_old_),2.000000000000000e+00f))+((trpnmax*kmtrpn)/powf((kmtrpn+cai_old_),2.000000000000000e+00f))));	//19
    real calc_I_katp = (fkatp*gkatp*calc_akik*calc_bkik*(v_old_-calc_EK));	//32
    real calc_th = (1.0f/(calc_ah+calc_bh));	//39
    real calc_jss = calc_hss;	//43
    real calc_tj = (1.0f/(calc_aj+calc_bj));	//44
    real calc_tiF = (calc_tiF_b*calc_delta_epi);	//70
    real calc_tiS = (calc_tiS_b*calc_delta_epi);	//71
    real calc_AiS = (1.0f-calc_AiF);	//73
    real calc_f = ((Aff*ff_old_)+(calc_Afs*fs_old_));	//98
    real calc_fcass = calc_fss;	//99
    real calc_Afcas = (1.0f-calc_Afcaf);	//103
    ...
    real calc_anca_ss = (1.0f/((k2n/calc_km2n)+powf((1.0f+(Kmn/cass_old_)),4.000000000000000e+00f)));	//116
    real calc_gamma_cass = expf(((-calc_constA)*4.000000000000000e+00f*((powf(calc_Iss,1.0/2.0)/(1.0f+powf(calc_Iss,1.0/2.0)))-(3.000000000000000e-01f*calc_Iss))));	//121
    real calc_gamma_cao = expf(((-calc_constA)*4.000000000000000e+00f*((powf(calc_Io,1.0/2.0)/(1.0f+powf(calc_Io,1.0/2.0)))-(3.000000000000000e-01f*calc_Io))));	//122
    real calc_gamma_nass = expf(((-calc_constA)*1.0f*((powf(calc_Iss,1.0/2.0)/(1.0f+powf(calc_Iss,1.0/2.0)))-(3.000000000000000e-01f*calc_Iss))));	//123
    real calc_gamma_nao = expf(((-calc_constA)*1.0f*((powf(calc_Io,1.0/2.0)/(1.0f+powf(calc_Io,1.0/2.0)))-(3.000000000000000e-01f*calc_Io))));	//124
    real calc_gamma_kss = expf(((-calc_constA)*1.0f*((powf(calc_Iss,1.0/2.0)/(1.0f+powf(calc_Iss,1.0/2.0)))-(3.000000000000000e-01f*calc_Iss))));	//125
    real calc_gamma_ko = expf(((-calc_constA)*1.0f*((powf(calc_Io,1.0/2.0)/(1.0f+powf(calc_Io,1.0/2.0)))-(3.000000000000000e-01f*calc_Io))));	//126
    real calc_PCap = (1.100000000000000e+00f*calc_PCa);	//131
    real calc_PCaNa = (1.250000000000000e-03f*calc_PCa);	//132
    real calc_PCaK = (3.574000000000000e+00f*calc_PCa);	//133
    real calc_anca_i = (1.0f/((k2n/calc_km2n)+powf((1.0f+(Kmn/cai_old_)),4.000000000000000e+00f)));	//140
    real calc_gamma_cai = expf(((-calc_constA)*4.000000000000000e+00f*((powf(calc_Ii,1.0/2.0)/(1.0f+powf(calc_Ii,1.0/2.0)))-(3.000000000000000e-01f*calc_Ii))));	//143
    real calc_gamma_nai = expf(((-calc_constA)*1.0f*((powf(calc_Ii,1.0/2.0)/(1.0f+powf(calc_Ii,1.0/2.0)))-(3.000000000000000e-01f*calc_Ii))));	//144
    real calc_gamma_ki = expf(((-calc_constA)*1.0f*((powf(calc_Ii,1.0/2.0)/(1.0f+powf(calc_Ii,1.0/2.0)))-(3.000000000000000e-01f*calc_Ii))));	//145
    ...
    real calc_beta_2 = (3.490000000000000e-01f*expf(((-1.062000000000000e+00f)*calc_vfrt)));	//158
    ...
    real calc_alpha_C2ToI = (5.200000000000000e-01f*expf((1.525000000000000e+00f*calc_vfrt)));	//161
    ...
    real calc_aK1 = (4.094000000000000e+00f/(1.0f+expf((1.217000000000000e-01f*((v_old_-calc_EK)-4.993400000000000e+01f)))));	//179
    real calc_bK1 = (((1.572000000000000e+01f*expf((6.740000000000000e-02f*((v_old_-calc_EK)-3.257000000000000e+00f))))+expf((6.180000000000000e-02f*((v_old_-calc_EK)-5.943099999999999e+02f))))/(1.0f+expf(((-1.629000000000000e-01f)*((v_old_-calc_EK)+1.420700000000000e+01f)))));	//180
    ...
    real calc_h6_i = (1.0f/calc_h4_i);	//191
    real calc_h11_i = ((nao*nao)/(calc_h10_i*kna1*kna2));	//196
    real calc_h12_i = (1.0f/calc_h10_i);	//197
    real calc_h5_ss = ((nass_old_*nass_old_)/(calc_h4_ss*kna1*kna2));	//227
    real calc_h6_ss = (1.0f/calc_h4_ss);	//228
    real calc_h11_ss = ((nao*nao)/(calc_h10_ss*kna1*kna2));	//233
    real calc_h12_ss = (1.0f/calc_h10_ss);	//234
    real calc_Knai = (Knai0*expf(((delta*calc_vfrt)/3.000000000000000e+00f)));	//259
    real calc_Knao = (Knao0*expf((((1.0f-delta)*calc_vfrt)/3.000000000000000e+00f)));	//260
    real calc_b3 = ((k3m*calc_P*H)/(1.0f+(MgATP/Kmgatp)));	//267
    real calc_IKb = (calc_GKb*calc_xkb*(v_old_-calc_EK));	//284
    real calc_INab = ((PNab*calc_vffrt*((nai_old_*expf(calc_vfrt))-nao))/(expf(calc_vfrt)-1.0f));	//285
    real calc_IClCa_junc = (((Fjunc*GClCa)/(1.0f+(KdClCa/cass_old_)))*(v_old_-calc_ECl));	//288
    real calc_IClCa_sl = ((((1.0f-Fjunc)*GClCa)/(1.0f+(KdClCa/cai_old_)))*(v_old_-calc_ECl));	//289
    real calc_IClb = (GClb*(v_old_-calc_ECl));	//291
    real calc_tau_rel = 0.0f;
    IFNUMBER_18(calc_tau_rel);	//299
    real calc_a_relp = ((5.000000000000000e-01f*calc_btp)/1.0f);	//302
    real calc_tau_relp_b = (calc_btp/(1.0f+(1.230000000000000e-02f/cajsr_old_)));	//305
    ...
    real calc_fINap = (1.0f/(1.0f+(KmCaMK/calc_CaMKa)));	//50
    real calc_fINaLp = (1.0f/(1.0f+(KmCaMK/calc_CaMKa)));	//61
    ...
    real calc_fItop = (1.0f/(1.0f+(KmCaMK/calc_CaMKa)));	//87
    real calc_fca = ((calc_Afcaf*fcaf_old_)+(calc_Afcas*fcas_old_));	//106
    real calc_fcap = ((calc_Afcaf*fcafp_old_)+(calc_Afcas*fcas_old_));	//114
    real calc_PhiCaL_ss = ((4.000000000000000e+00f*calc_vffrt*((calc_gamma_cass*cass_old_*expf((2.000000000000000e+00f*calc_vfrt)))-(calc_gamma_cao*cao)))/(expf((2.000000000000000e+00f*calc_vfrt))-1.0f));	//127
    real calc_PhiCaNa_ss = ((1.0f*calc_vffrt*((calc_gamma_nass*nass_old_*expf((1.0f*calc_vfrt)))-(calc_gamma_nao*nao)))/(expf((1.0f*calc_vfrt))-1.0f));	//128
    real calc_PhiCaK_ss = ((1.0f*calc_vffrt*((calc_gamma_kss*kss_old_*expf((1.0f*calc_vfrt)))-(calc_gamma_ko*ko)))/(expf((1.0f*calc_vfrt))-1.0f));	//129
    real calc_PCaNap = (1.250000000000000e-03f*calc_PCap);	//134
    real calc_PCaKp = (3.574000000000000e+00f*calc_PCap);	//135
    real calc_fICaLp = (1.0f/(1.0f+(KmCaMK/calc_CaMKa)));	//136
    real calc_PhiCaL_i = ((4.000000000000000e+00f*calc_vffrt*((calc_gamma_cai*cai_old_*expf((2.000000000000000e+00f*calc_vfrt)))-(calc_gamma_cao*cao)))/(expf((2.000000000000000e+00f*calc_vfrt))-1.0f));	//146
    real calc_PhiCaNa_i = ((1.0f*calc_vffrt*((calc_gamma_nai*nai_old_*expf((1.0f*calc_vfrt)))-(calc_gamma_nao*nao)))/(expf((1.0f*calc_vfrt))-1.0f));	//147
    real calc_PhiCaK_i = ((1.0f*calc_vffrt*((calc_gamma_ki*ki_old_*expf((1.0f*calc_vfrt)))-(calc_gamma_ko*ko)))/(expf((1.0f*calc_vfrt))-1.0f));	//148
    ...
    real calc_h1_i = (1.0f+((nai_old_/kna3)*(1.0f+calc_hna)));	//186
    real calc_h7_i = (1.0f+((nao/kna3)*(1.0f+(1.0f/calc_hna))));	//192
    real calc_k1_i = (calc_h12_i*cao*kcaon);	//198
    real calc_k6_i = (calc_h6_i*cai_old_*kcaon);	//207
    real calc_h1_ss = (1.0f+((nass_old_/kna3)*(1.0f+calc_hna)));	//223
    real calc_h7_ss = (1.0f+((nao/kna3)*(1.0f+(1.0f/calc_hna))));	//229
    real calc_k1_ss = (calc_h12_ss*cao*kcaon);	//235
    real calc_k6_ss = (calc_h6_ss*cass_old_*kcaon);	//244
    real calc_a1 = ((k1p*powf((nai_old_/calc_Knai),3.000000000000000e+00f))/((powf((1.0f+(nai_old_/calc_Knai)),3.000000000000000e+00f)+powf((1.0f+(ki_old_/Kki)),2.000000000000000e+00f))-1.0f));	//262
    real calc_b2 = ((k2m*powf((nao/calc_Knao),3.000000000000000e+00f))/((powf((1.0f+(nao/calc_Knao)),3.000000000000000e+00f)+powf((1.0f+(ko/Kko)),2.000000000000000e+00f))-1.0f));	//265
    real calc_a3 = ((k3p*powf((ko/Kko),2.000000000000000e+00f))/((powf((1.0f+(nao/calc_Knao)),3.000000000000000e+00f)+powf((1.0f+(ko/Kko)),2.000000000000000e+00f))-1.0f));	//266
    real calc_b4 = ((k4m*powf((ki_old_/Kki),2.000000000000000e+00f))/((powf((1.0f+(nai_old_/calc_Knai)),3.000000000000000e+00f)+powf((1.0f+(ki_old_/Kki)),2.000000000000000e+00f))-1.0f));	//269
    real calc_ICab = ((PCab*4.000000000000000e+00f*calc_vffrt*((calc_gamma_cai*cai_old_*expf((2.000000000000000e+00f*calc_vfrt)))-(calc_gamma_cao*cao)))/(expf((2.000000000000000e+00f*calc_vfrt))-1.0f));	//286
    real calc_IClCa = (calc_IClCa_junc+calc_IClCa_sl);	//290
    real calc_tau_relp = 0.0f;
    IFNUMBER_20(calc_tau_relp);	//306
    real calc_fJrelp = (1.0f/(1.0f+(KmCaMK/calc_CaMKa)));	//308
    real calc_fJupp = (1.0f/(1.0f+(KmCaMK/calc_CaMKa)));	//313
    real calc_INa = (GNa*(v_old_-calc_ENa)*powf(m_old_,3.000000000000000e+00f)*(((1.0f-calc_fINap)*h_old_*j_old_)+(calc_fINap*hp_old_*jp_old_)));	//51
    real calc_INaL = (calc_GNaL*(v_old_-calc_ENa)*mL_old_*(((1.0f-calc_fINaLp)*hL_old_)+(calc_fINaLp*hLp_old_)));	//62
    real calc_Ito = (calc_Gto*(v_old_-calc_EK)*(((1.0f-calc_fItop)*a_old_*calc_i)+(calc_fItop*ap_old_*calc_ip)));	//88
    real calc_ICaL_ss = (ICaL_fractionSS*(((1.0f-calc_fICaLp)*calc_PCa*calc_PhiCaL_ss*d_old_*((calc_f*(1.0f-nca_ss_old_))+(jca_old_*calc_fca*nca_ss_old_)))+(calc_fICaLp*calc_PCap*calc_PhiCaL_ss*d_old_*((calc_fp*(1.0f-nca_ss_old_))+(jca_old_*calc_fcap*nca_ss_old_)))));	//137
    real calc_ICaNa_ss = (ICaL_fractionSS*(((1.0f-calc_fICaLp)*calc_PCaNa*calc_PhiCaNa_ss*d_old_*((calc_f*(1.0f-nca_ss_old_))+(jca_old_*calc_fca*nca_ss_old_)))+(calc_fICaLp*calc_PCaNap*calc_PhiCaNa_ss*d_old_*((calc_fp*(1.0f-nca_ss_old_))+(jca_old_*calc_fcap*nca_ss_old_)))));	//138
    real calc_ICaK_ss = (ICaL_fractionSS*(((1.0f-calc_fICaLp)*calc_PCaK*calc_PhiCaK_ss*d_old_*((calc_f*(1.0f-nca_ss_old_))+(jca_old_*calc_fca*nca_ss_old_)))+(calc_fICaLp*calc_PCaKp*calc_PhiCaK_ss*d_old_*((calc_fp*(1.0f-nca_ss_old_))+(jca_old_*calc_fcap*nca_ss_old_)))));	//139
    real calc_ICaL_i = ((1.0f-ICaL_fractionSS)*(((1.0f-calc_fICaLp)*calc_PCa*calc_PhiCaL_i*d_old_*((calc_f*(1.0f-nca_i_old_))+(jca_old_*calc_fca*nca_i_old_)))+(calc_fICaLp*calc_PCap*calc_PhiCaL_i*d_old_*((calc_fp*(1.0f-nca_i_old_))+(jca_old_*calc_fcap*nca_i_old_)))));	//149
    real calc_ICaNa_i = ((1.0f-ICaL_fractionSS)*(((1.0f-calc_fICaLp)*calc_PCaNa*calc_PhiCaNa_i*d_old_*((calc_f*(1.0f-nca_i_old_))+(jca_old_*calc_fca*nca_i_old_)))+(calc_fICaLp*calc_PCaNap*calc_PhiCaNa_i*d_old_*((calc_fp*(1.0f-nca_i_old_))+(jca_old_*calc_fcap*nca_i_old_)))));	//150
    real calc_ICaK_i = ((1.0f-ICaL_fractionSS)*(((1.0f-calc_fICaLp)*calc_PCaK*calc_PhiCaK_i*d_old_*((calc_f*(1.0f-nca_i_old_))+(jca_old_*calc_fca*nca_i_old_)))+(calc_fICaLp*calc_PCaKp*calc_PhiCaK_i*d_old_*((calc_fp*(1.0f-nca_i_old_))+(jca_old_*calc_fcap*nca_i_old_)))));	//151
    ...
    real calc_Jrel = (Jrel_b*(((1.0f-calc_fJrelp)*Jrel_np_old_)+(calc_fJrelp*Jrel_p_old_)));	//309
    real calc_Jup = (Jup_b*((((1.0f-calc_fJupp)*calc_Jupnp)+(calc_fJupp*calc_Jupp))-calc_Jleak));	//315
    ...
    real calc_Jrel_inf_b = ((((-calc_a_rel)*calc_ICaL_ss)/1.0f)/(1.0f+powf((cajsr_half/cajsr_old_),8.000000000000000e+00f)));	//296
    real calc_Jrel_infp_b = ((((-calc_a_relp)*calc_ICaL_ss)/1.0f)/(1.0f+powf((cajsr_half/cajsr_old_),8.000000000000000e+00f)));	//303
    ...
    real calc_INaCa_i = ((1.0f-INaCa_fractionSS)*calc_Gncx*calc_allo_i*((zna*calc_JncxNa_i)+(zca*calc_JncxCa_i)));	//222
